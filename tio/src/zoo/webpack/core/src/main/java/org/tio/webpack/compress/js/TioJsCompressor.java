/*
 * ajsgkvr本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动votbaufaazvx
 */
...
/**
 * An example of how to call the Closure Compiler programmatically, this is
 * LICENSED AS GPL-3.0.
 *
...
public class TioJsCompressor implements ResCompressor {
     ...
     * @author tanyaowu
     */
    public static String compress(String srcFilePath, String srcFileContent, LanguageMode languageMode,
	...
	try {
	    ...
	    try {
		list = AbstractCommandLineRunner.getBuiltinExterns(CompilerOptions.Environment.BROWSER);
		...
		if (ret == null || ret.length() == 0) {
		    ...
		    // log.warn("用{}语言模式压缩后的文件大小为0，换ECMASCRIPT_NEXT试试, {}", languageMode,
		    // srcFilePath);
		    // return compress(srcFilePath, srcFileContent, LanguageMode.ECMASCRIPT_NEXT,
			...
			return compress(srcFilePath, srcFileContent, languageMode,
				CompilationLevel.SIMPLE_OPTIMIZATIONS);
		...
		// log.warn("压缩后的文件反而较大, init size:{}, after size:{}, file:{}",
		// initBytes.length, afterBytes.length, srcFilePath);
		// return srcFileContent;
		// }
	...
	try {
	    ...
	    // String content = FileUtil.readString(new
	    // File("E:\\svn\\nbyb\\html\\nbyb_bootstrap\\src\\public\\js\\jquery-3.2.1.min.js"),
	    // "utf-8");
	    // File initFile = new
	...
	return compress(srcFilePath, srcFileContent, LanguageMode.ECMASCRIPT_NEXT,
		CompilationLevel.SIMPLE_OPTIMIZATIONS);
	// return compress(srcFilePath, srcFileContent,
