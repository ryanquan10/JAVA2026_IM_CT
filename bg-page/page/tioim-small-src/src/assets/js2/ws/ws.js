...
function initWs() {
	...
	tiows = new tioWs(
		async function (reconn) {
			...
			console.log('bs_tio_session', bs_tio_session);
			if (_tio_session) {
			...
			var queryString = "wx=1&" + sessionName + '=' + encodeURIComponent(sessionValue);console.log('hear', queryString);
			...
			var q1 = 'wx=1&tio_session=1650800770725060608&frompath=%2Fhome';
			var url = ws_protocol + '://' + ws_server + ':' + ws_port + '?' + queryString;

			if (reconn) {
				url += '&tiows_reconnect=true';
			}
			console.log('hear',{ url: url, timeout: heartbeatTimeout });
			// return url;
