      ...
      >
        ...
        <el-table-column label="是否公开" prop="isOpen">
          <template slot-scope="scope">
            <span class="wx-color" v-if="scope.row.is_open == 1">公开</span>
            <span class="c-red" v-else>私有</span>
          </template>
        </el-table-column>
        <el-table-column label="审核加入" prop="isExamine">
          <template slot-scope="scope">
            <span class="wx-color" v-if="scope.row.is_examine == 1">是</span>
            <span class="c-red" v-else>否</span>
          </template>
        </el-table-column>
        <el-table-column label="是否开启邀请码" prop="isInvite">
          <template slot-scope="scope">
            <span class="wx-color" v-if="scope.row.is_invite == 1">是</span>
            <span class="c-red" v-else>否</span>
          </template>
        </el-table-column>
        <!-- <el-table-column label="支付方式">
                    <template slot-scope="scope">
                        ...
                        <span class="tmopera" v-else>支付宝支付</span>
                    </template>
                </el-table-column> -->
        <el-table-column
          label="创建时间"
          ...
          <template slot-scope="scope">
            <template>
              <span
                ...
                >审核</span
              >
              <a v-else class="tmopera" @click="setData('see', scope.row)"
                >查看</a
              >
...
export default {
  ...
  methods: {
    ...
    getData(item) {
      ...
      circleCenter.list(ptdata).then((res) => {
        if (res.ok) {
          let data = res.data;
          if (data) {
