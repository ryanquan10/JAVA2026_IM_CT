/*
 * zvbftm本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动czkzabfem
 */
...
public class DefaultHttpRequestHandler implements HttpRequestHandler {
	...
	/**
	 * 静态资源的CacheName key: path 譬如"/index.html" value: FileCache
	 */
	...
	public HttpResponse handler(HttpRequest request) throws Exception {
		...
		try {
							...
							if (paramType == HttpRequest.class) {
								...
								if (params != null) {
									if (ClassUtils.isSimpleTypeOrArray(paramType)) {
										...
										paramValues[i] = paramType.newInstance();// BeanUtil.mapToBean(params,
										// paramType, true);
												...
												if (propertyDescriptor == null) {
													...
													if (ClassUtils.isSimpleTypeOrArray(clazz)) {
														if (fieldValue != null && fieldValue.length > 0) {
															if (clazz.isArray()) {
																Object theValue = null;// Convert.convert(clazz,
																// fieldValue);
					...
					if (pageRoot != null) {
						...
						if (httpResource != null) {
							...
							if (freemarkerConfig != null) {
								if (ArrayUtil.contains(freemarkerConfig.getSuffixes(), extension) && !freemarkerConfig.skipFk(path)) {
										...
										if (request.isClosed()) {
											...
											if (model instanceof HttpResponse) {
												...
												try {
													...
													freemarkerConfig.getSkipPathItems().add(path);
							...
							if (file != null) {
								...
								response = Resps.bytes(request, IoUtils.toByteArray(inputStream), extension);// .file(request,
								// file);
							}
			...
			response = resp404(request, requestLine);// Resps.html(request, "404--并没有找到你想要的内容",
			// httpConfig.getCharset());
			return response;
		} catch (Throwable e) {
			logError(request, requestLine, e);
			response = resp500(request, requestLine, e);// Resps.html(request, "500--服务器出了点故障",
			// httpConfig.getCharset());
			return response;
		} finally {
			try {
	...
	private void processCookieAfterHandler(HttpRequest request, RequestLine requestLine, HttpResponse httpResponse) throws ExecutionException {
		...
		HttpSession httpSession = request.getHttpSession();// (HttpSession)
		// channelContext.get();//.getHttpSession();//not null
	...
	private boolean statIpPath(HttpRequest request, HttpResponse response, String path, long iv) {
		...
		for (Long duration : ipPathAccessStats.durationList) {
			IpAccessStat ipAccessStat = ipPathAccessStats.get(duration, ip);// .get(duration, ip, path);//.get(v,
	...
	private boolean statTokenPath(HttpRequest request, HttpResponse response, String path, long iv) {
		...
		if (StrUtil.isNotBlank(token)) {
			...
			for (Long duration : list) {
				TokenAccessStat tokenAccessStat = tokenPathAccessStats.get(duration, token, request.getClientIp(), uid);// .get(duration,
				// ip,
				// path);//.get(v,
				// channelContext.getClientNode().getIp());
	 ...
	 * @author tanyaowu
	 */
	public HttpSession updateSessionId(HttpRequest request, HttpSession newSession, HttpResponse httpResponse) {
		return updateSessionId(request, newSession, httpResponse, null);
	}
	 ...
	 * @author tanyaowu
	 */
	public HttpSession updateSessionId(HttpRequest request, HttpSession newSession, HttpResponse httpResponse, String newId) {
		String oldId = request.getHttpSession().getId();
		if (cn.hutool.core.util.StrUtil.isBlank(newId)) {
			newId = httpConfig.getSessionIdGenerator().sessionId(httpConfig, request);
			if (httpSessionListener != null) {
				httpSessionListener.doAfterCreated(request, newSession, httpConfig);
			}
		}
...
//		HttpSession oldHttpSession = (HttpSession) httpConfig.getSessionStore().get(oldId);
//		if (oldHttpSession != null) {
//			oldHttpSession.clear(httpConfig);
//			oldHttpSession.update(httpConfig); //切记：每次修改SessionExt后，要调用一下update把数据更新到分布式缓存中
//		}
		httpConfig.getSessionStore().remove(oldId);
		
		createSessionCookie(request, newSession, httpResponse, true);
		newSession.update(httpConfig); // HttpSession有变动时，都要调一下update()
		return newSession;
	}
}
