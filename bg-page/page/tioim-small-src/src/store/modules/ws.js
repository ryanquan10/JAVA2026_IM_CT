...
import { friend, chatcom, chatInfo, circle, msgTips } from '@/axios/path';
...
const state = {
    ...
    isConnect: false,
    isHomeReq: false, // 是否已经请求数据 进入首页需要页面挂载之后请求数据
    ...
    notReadCircleNoticeNum: 0,
    newCircleNotice: null
};
const actions = {
    /* 申请好友列表 -
        1.当处于申请列表页面，赋值列表数据 
    ...
    */
    ...
    // 获取未读消息
    getNotReadNoticeNum({ state }) {
        circle.notReadNoticeNum().then(res => {
            if (res.ok) {
               state.notReadCircleNoticeNum = res.data.unreadCount;
            }
        })
    },
...
const mutations = {
    ...
    chatMessageCt(state, { bodyObj, fromuser, sendtype, unshift }) {
        ...
        if(bodyObj.ct == 66){
            bodyObj.ct = 1;
            bodyObj.isQuote = true;
            if([1, 2].includes(Number(bodyObj.quotemsgtype))){
                //内容换行
                let quotehtml = bodyObj.quotemsgcontent.replace(/\n/g, '<br>');
                //表情编译-begin
                quotehtml = messageEmoji(quotehtml);
                bodyObj.quotemsgcontent = quotehtml;
            }else{
                bodyObj.quotemsgcontent = JSON.parse(bodyObj.quotemsgcontent)
            }
        }
        let ct = bodyObj.ct;
        let bodyhtml = "";
        ...
        // 1、普通文本消息，2、超链接卡片消息，3、文件，4、音频，5、视频 ,6.图片 ,9.名片,10.视频通话,11.音频通话 新拓展：20云相册
        let bodyc = bodyObj.c;
        ...
        switch (ct) {
            ...
            //     //表情编译-begin
            //     bodyhtml = messageEmoji(bodyhtml);
            //     if(bodyObj.quotemsgtype == 1 || bodyObj.quotemsgtype == 2){
            //         //网址处理
            //         let reg1 = /(http:\/\/|https:\/\/)((\w|=|\?|\.|\/|&|-|:)+)/g;
            //         bodyhtml = bodyhtml.replace(reg1, "<a href='$1$2' target='_blank' class='texthttp'>$1$2</a>");
            //     }else{
            //         bodyObj.quotemsgcontent = JSON.parse(bodyObj.quotemsgcontent)
            //     }
            //     break;
    ...
    */
    ...
    currentPage(state) {
        ...
        var onVisibilityChange = function () {
            if (!document[hiddenProperty]) {
                ...
                $("#tiotitle").html(currUser ? currUser.nick : tioConfig.tioim_title);//标题
                //iframe父页面标题
                if ($(window.parent.document).find("#tioim-container-iframe")) {
    ...
    setNoReadApply(state, val) {
     ...
     /* 获取最后一条朋友圈信息，用于实现已读未读 */
    setLastCircle(state,val){
        console.log('setLastCircle');
        state.lastCircle = val;
        console.log(state.lastCircle);
    },
     /* 将本地朋友圈最后一条信息设为已读 */
    localReadCircle(state){
        if(state.lastCircle && state.lastCircle.readflag != 1){
            state.lastCircle.readflag = 1;
        }
    },
    // 设置朋友圈通知信息
    setCircleNotice(state, val){
        state.circleNotice = val;
    },
    // 设置朋友圈消息未读条数
    setNotReadCircleNoticeNum(state, val){
        state.notReadCircleNoticeNum =val;
    },
    addNotReadCircleNoticeNum(state){
        state.notReadCircleNoticeNum++;
    },
    // 最新的朋友圈推送
    setNewCircleNotice(state, val){
        state.newCircleNotice = val;
    },
