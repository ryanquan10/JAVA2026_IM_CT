  ...
  <div class="commonright container groupChatMage">
    ...
    <div class="contentpad">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.vnum }}</span>
            <i
              class="edit-vnum el-icon-edit"
              @click="openEditVnum(scope.row)"
            ></i>
          </template>
        </el-table-column>
        <el-table-column label="群主" :align="$protovar.align" width="180">
          <template slot-scope="scope">
            <div class="user">
              <div>
                <el-image
                  @click="seeInfo(scope.row, 'groupMage')"
        ...
        <el-table-column label="管理员" :align="$protovar.align">
          <template slot-scope="scope">
            <span
              class="color_3A7BE9 cursor"
              @click="seeforbiddenList(scope.row, 'manageList')"
              >查看</span
            >
          </template>
        </el-table-column>
        ...
        <el-table-column label="禁言名单" :align="$protovar.align">
          <template slot-scope="scope">
            <span
              class="color_3A7BE9 cursor"
              @click="seeforbiddenList(scope.row, 'forbiddenList')"
              >查看</span
            >
          </template>
        </el-table-column>
        <el-table-column label="投诉记录" :align="$protovar.align">
          <template slot-scope="scope">
            <span class="color_3A7BE9 cursor" @click="seereportList(scope.row)"
              >查看</span
            >
          </template>
        </el-table-column>
        <el-table-column label="停封记录" :align="$protovar.align">
          <template slot-scope="scope">
            <span
              class="color_3A7BE9 cursor"
              @click="seeforbiddenList(scope.row, 'inblackList')"
              >查看</span
            >
          </template>
        </el-table-column>
        <el-table-column label="聊天记录" :align="$protovar.align">
          <template slot-scope="scope">
            <span @click="showMsgList(scope.row)" class="color_3A7BE9 cursor"
              >查看</span
            >
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" width="120">
          <template slot-scope="scope">
            <div style="display: flex; justify-content: center">
              <span
                class="tmopera"
                @click="inblackclick(scope.row)"
                v-auth="'reset'"
                >{{ scope.row.status == 1 ? "封停" : "解封" }}</span
              >
            </div>
          </template>
        </el-table-column>
    ...
    <!-- 封停/解封弹窗 -->
    <el-dialog
      :visible.sync="visible.groupChatShow"
    ...
    >
      <div class="groupChatBox">
        ...
        <textarea
          v-model="inblack.reason"
          :placeholder="groupStatus == 1 ? '请输入封停原由' : '请输入解封原由'"
        ></textarea>
      ...
      <div class="tmdialog-footer">
        <button
          ...
          @click="visible.groupChatShow = false"
        >
          取消
        </button>
    ...
    <!-- 禁言名单/管理员名单/解停封记录 -->
    <el-dialog
      :visible.sync="visible.tableListShow"
      class="forbiddenList-dialog"
      width="1065px"
    >
      <div class="forbiddenListBox">
        ...
        <div class="forbiddenList-header">
          ...
          <span class="refresh" @click="refreshMsg">
            <img
              src="~@_/assets/img/common/refreshin.png"
        ...
        <div class="forbiddenList-table">
          ...
          >
            ...
            <div class="table-col" v-show="popfrom == 'forbiddenList'">
              <span class="color_666"
                >{{ item.cancelforbiddentimeday || "-" }} </span
              ><br />
              <span class="color_999"
                >{{ item.cancelforbiddentime || "-" }}
              </span>
            </div>
            <div class="table-col" v-show="popfrom == 'forbiddenList'">
            ...
            <div class="table-col" v-show="popfrom == 'manageList'">
              <span class="color_666">{{ item.setroletimeday || "-" }} </span
              ><br />
            ...
            <div class="table-col" v-show="popfrom == 'inblackList'">
              <span
                :class="[item.oper == 1 ? 'tmdisabledtrue' : 'tmdisabledfalse']"
                >{{ item.oper == 1 ? "封停" : "解封" }}
              </span>
            </div>
            <div class="table-col" v-show="popfrom == 'inblackList'">
            ...
            <div class="table-col" v-show="popfrom == 'inblackList'">
              <span class="color_666">{{ item.createtimeday || "-" }} </span
              ><br />
          ...
          <div class="forbiddenList-li" v-show="table.data.length <= 0">
            暂无记录
          </div>
        </div>
      </div>
    ...
    <!-- 群举报列表 -->
    <el-dialog
      :visible.sync="visible.complaintShow"
      class="complaintShow-dialog"
      width="1080px"
    >
      <div class="complaintShowBox">
        ...
        <div class="complaintShow-header">
          ...
          <div class="filter">
            ...
            <div class="filter-btn">
              <button class="primarybtn search" @click="getreportlist(1)">
                查询
              </button>
        ...
        <div class="complaintShow-table">
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <div class="reason-box">
                  <p
                    class="reason"
                    @mouseover="reasonmouse(scope.row)"
                    @mouseout="visible.allReasonShow = false"
                  >
                    {{ scope.row.reason }}
                  </p>
                </div>
            ...
            <el-table-column label="图片">
              <template slot-scope="scope">
                <span
                  ...
                  class="tmopera"
                  @click="previewImgs(scope.row.imgs)"
                  >{{ scope.row.imgs.length }}张</span
                >
            ...
            >
              <template slot-scope="scope">
                <span class="color_666">{{ scope.row.createtimeday }} </span
                ><br />
                <span class="color_999">{{ scope.row.createtime }} </span>
              </template>
            </el-table-column>
            ...
            <el-table-column label="终端" :align="$protovar.align" width="120">
              <template slot-scope="scope">
                <div class="devicetype">
                  <p>
                    <img
                      class="devicetype_Img"
                      :src="scope.row.devicetypeImg"
                      alt=""
                      srcset=""
                    />
    ...
    <!-- 举报标记 -->
    <el-dialog
      :visible.sync="visible.confirmReportdeal"
      :show-close="false"
      class="confirmReportdeal-dialog"
      width="400px"
    >
      ...
      <div class="tmdialog-footer">
        <button
          class="primarybtn search"
          @click="visible.confirmReportdeal = false"
        >
          取消
        </button>
    ...
    >
            ...
            <div class="top-center">
              <p class="group-info">
                ...
                <span
                  :class="[
                    'group-status',
                    info.data.status == 1 ? '' : 'group-status-error',
                  ]"
                  >{{ info.data.status == 1 ? "有效群" : "无效群" }}</span
                >
              </p>
              <div class="fidgop">{{ info.data.joinnum }}位成员</div>
            </div>
            <div class="top-right">
              <button class="primarybtn" @click="showMsgList(info.data)">
                查看记录
              </button>
            </div>
          </div>
      ...
      <div class="info-content">
        ...
        <p class="info-col">
          <span>成员邀请：</span>
          <span class="info-row">{{
            info.data.applyflag == 1 ? "开启" : "已关闭"
          }}</span>
        </p>
        <p class="info-col">
          <span>进群审核：</span>
          <span class="info-row">{{
            info.data.joinmode == 1 ? "开启" : "已关闭"
          }}</span>
        </p>
        <p class="info-col">
          <span>全员禁言：</span>
          <span class="info-row">{{
            info.data.forbiddenflag == 1 ? "开启" : "已关闭"
          }}</span>
        </p>
        <!-- <p class="info-col">
                    <span>近1月消息数：</span>
      ...
      <div class="info-bottom">
        <button
          v-if="info.data.isdefault == 1"
          class="primarybtn auto red"
          @click="changeDefaultGroupStatus(info.data)"
        >
          取消默认群组
        </button>
        <button
          v-else
          class="primarybtn auto"
          @click="changeDefaultGroupStatus(info.data)"
        >
          添加到默认群组
        </button>
    ...
    <!-- 修改虚拟人员 -->
    <el-dialog
      :visible.sync="dialogVnum.visible"
      :show-close="false"
      class="confirmReportdeal-dialog"
      width="400px"
      :title="dialogVnum.title"
    >
      <div class="vnum-main">
        <el-input
          class="text-c"
          v-model="dialogVnum.form.vnum"
          placeholder="请输入"
        ></el-input>
      </div>
      <div class="row-c-c">
        <button class="primarybtn search" @click="dialogVnum.visible = false">
          取消
        </button>
        <button class="primarybtn" @click="editVnum()">保存</button>
      </div>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    setScroll() {
      ...
      $chatmsglist.on("scroll", async () => {
        if (
          this.table.loading &&
          this.table.totalPage > this.table.pageNumber &&
          $chatmsglist.scrollTop() == 0
        ) {
          ...
          this.$nextTick(() => {
            //重定位滚动位置
            let topOffsetPx =
              $(".forbiddenList-table").offset().top - $chatmsglist.height();
    ...
    // 修改默认群组状态
    changeDefaultGroupStatus(group) {
      let params = {
        groupid: group.id,
      };
      let isdefault = group.isdefault == 0? true : false;
      let request = isdefault ? defaultGroups.add : defaultGroups.delete;
    ...
    openEditVnum(item) {
      this.dialogVnum.visible = true;
      this.dialogVnum.form = item;
    },
    editVnum() {
      let form = this.dialogVnum.form;
      let params = {
        groupid: form.id,
        vnum: form.vnum,
      };
      newgroupList.updateVnum(params).then((res) => {
        this.getData();
        this.dialogVnum.visible = false;
        successTips("操作成功！");
      });
    },
  },
