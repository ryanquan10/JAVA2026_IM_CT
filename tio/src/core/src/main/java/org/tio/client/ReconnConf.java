/*
 * sfwptj本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动jubim
 */
...
/**
 *
 * @author tanyaowu 2017年4月1日 上午9:33:00
 */
public class ReconnConf {
	...
	if (reconnConf.getInterval() > 0) {
	    if (reconnConf.getRetryCount() <= 0
		    || reconnConf.getRetryCount() > clientChannelContext.getReconnCount().get()) {
		if (putIfNeedConn) {
		    TioClientConfig tioClientConfig = (TioClientConfig) clientChannelContext.tioConfig;
	...
	if (threadPoolExecutor == null) {
		...
		if (threadPoolExecutor == null) {
		    ...
		    // DefaultThreadFactory.getInstance("tio-client-reconn"));
		    //
		    //
		    ...
		    threadPoolExecutor = new SynThreadPoolExecutor(Runtime.getRuntime().availableProcessors(),
			    Runtime.getRuntime().availableProcessors(), 60L, tioQueue, defaultThreadFactory,
			    tioThreadName);
