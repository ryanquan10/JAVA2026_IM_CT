/*
 * dddmtrrffo本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动ozzquaxmu
 */
...
/**
 * @author tanyaowu 2017年8月12日 下午9:13:54
 */
public class CaffeineRedisCache extends AbsCache {
	...
	if (caffeineRedisCache == null && !skipNull) {
	    log.warn("cacheName[{}]还没注册，请初始化时调用：{}.register(cacheName, timeToLiveSeconds, timeToIdleSeconds)",
		    cacheName, CaffeineRedisCache.class.getSimpleName());
	}
	...
	if (!inited) {
		...
		if (!inited) {
			...
			public void onMessage(CharSequence channel, CacheChangedVo cacheChangedVo) {
			    ...
			    if (type == CacheChangeType.PUT || type == CacheChangeType.UPDATE
				    || type == CacheChangeType.REMOVE) {
    ...
    public static CaffeineRedisCache register(RedissonClient redisson, String cacheName, Long timeToLiveSeconds,
	...
	if (caffeineRedisCache == null) {
		...
		if (caffeineRedisCache == null) {
		    RedisCache redisCache = RedisCache.register(redisson, cacheName, timeToLiveSeconds,
			    timeToIdleSeconds);
		    ...
		    CaffeineCache caffeineCache = CaffeineCache.register(cacheName, timeToLiveSecondsForCaffeine,
	...
	//
	// CacheChangedVo cacheChangedVo = new CacheChangedVo(cacheName, key,
	// CacheChangeType.PUT);
	// topic.publish(cacheChangedVo);
    }
