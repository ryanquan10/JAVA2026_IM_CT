...
public class UploadController {
	 ...
	 * @author tanyaowu
	 */
	@RequestPath(value = "/db")
	public List<Record> db(HttpRequest request, String type) throws Exception {
		if (!IpWhiteListService.isWhiteIp(request.getClientIp())) {
			return null;
		}
		String sql = "select * from " + type + " where updatetime > ?";
	...
	public List<Record> xx(HttpRequest request, String sql) throws Exception {
		long iv = 3600L * 24L * 1000L * 30L * 6; // 只取最近6个月的
