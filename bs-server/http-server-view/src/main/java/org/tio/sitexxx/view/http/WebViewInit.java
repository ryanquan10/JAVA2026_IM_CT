...
public class WebViewInit {
	...
	public static void initWsBs(String pageRoot) {
		initHandlers(pageRoot, "tx/ws/bs", "tx/ws/bs.js");
	}
	
	private static void initHandlers(String pageRoot, String bsdir, String bsjs) {
		File wxbsDir = new File(pageRoot, bsdir);
		...
		if (ArrayUtil.isNotEmpty(files)) {
			for (int i = 0; i < files.length; i++) {
				...
				File file = files[i];
				sb.append("<#include \"/"+bsdir+"/").append(FileUtil.getName(file)).append("\" >").append("\r\n\r\n");
			}
		}

		FileUtil.writeUtf8String(sb.toString(), new File(pageRoot, bsjs));
	}
	...
	public static void init(Root model, ModelGenerator modelGenerator) throws Exception {
		...
		if (StrUtil.isNotBlank(skipFkPathStr)) {
			String[] skipFkPathArray = StrUtil.splitToArray(skipFkPathStr, ";");
