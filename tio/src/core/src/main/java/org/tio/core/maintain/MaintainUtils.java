/*
 * lbaidtxgb本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动neldorimpfux
 */
...
import org.tio.core.ChannelContext.RemoveListener;
import org.tio.utils.hutool.FileUtil;
import org.tio.utils.lock.ReadLockHandler;

/**
 * 
 * @author tanyaowu 2017年10月19日 上午9:40:34
 */
public class MaintainUtils {
	...
	public static void remove(ChannelContext channelContext) {
		if (channelContext.removeListeners != null) {
			channelContext.removeListeners.handle(new ReadLockHandler<Set<RemoveListener>>() {
				@Override
				public void handler(Set<RemoveListener> set) {
					for (RemoveListener removeListener : set) {
						try {
							removeListener.onRemove(channelContext);
						} catch (Exception e) {
							log.error("", e);
						}
					}
