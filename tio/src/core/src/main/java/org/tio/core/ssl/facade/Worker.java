/*
 * mlqiyv本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动yzyozlz
 */
...
class Worker {
    ...
    /*
     * Uses the SSLEngine and Buffers to perform wrap/unwrap operations. Also,
     * provides access to SSLEngine ops for handshake
     */
	...
	try {
		...
		log.error(channelContext + ", 解密Error:" + e.toString() + ", byte:" + StrUtil.arrayToString(bs)
			+ ", string:" + new String(bs) + ", buffer:" + cipherText, e);
	    }
	    throw e;
	}
    }
