<template>
  <div class="navbar">
    <div class="nav-top user">
      ...
      <!-- 用户信息 -->
      <div class="userinfo" @click.stop="stopProp" v-show="show">
        <div class="baseinfo">
          <img :src="currUser.avatar" class="base-bg" />
          <div class="info-avatar">
            <el-image class="info-avatar" :src="currUser.avatar">
              <div slot="error" class="image-slot">
                <img src="~@/assets/imgs/common/avatar.jpg" class="error-img" />
              </div>
            </el-image>
            <span class="hoverblack" @click="showUpdateInfo(2)"></span>
            <img
          ...
          <!-- <p class="user-loginname">{{ currUser.loginname }}{{currUser}}</p> -->
          <p class="user-loginname">{{ currUser.id }}</p>
          <div class="qrCode" @click="infoQrCode = !infoQrCode"></div>
        </div>
        <div class="detail-info">
          ...
          <div class="info-item" v-if="switchConfig.isOpenAddressShow.value">
            <label class="info-item_label">地区</label>
            <span class="info-item_col"
              >{{ currUser.province }} {{ currUser.city }}</span
            >
          </div>
          <div class="info-item">
        ...
        <div class="info-group-btn">
          <button class="primarybtn" @click="showUpdateInfo(1)">
            修改信息
          </button>
          <button class="primarybtn default" @click="sendMsg">发消息</button>
        </div>
      </div>
    </div>
    <!-- 路由tab -->
    <div class="nav-center">
      <div
        :class="[
          'navbar-icon navbar-chat',
          currentRoute == '/home' ? 'active' : '',
        ]"
        @click="toRoute('/home')"
      >
        <span>聊天</span>
        <span v-if="allNotRead != 0" class="notread notreadwid">{{
          allNotRead
        }}</span>
      </div>
      <div
        :class="[
          'navbar-icon navbar-friends',
          currentRoute == '/friend' ? 'active' : '',
        ]"
        @click="toRoute('/friend')"
      >
        <span>好友</span>
        <span v-if="noReadApply != 0" class="notread notreadwid">{{
          noReadApply
        }}</span>
      </div>
      <div
        :class="[
          'navbar-icon navbar-groups',
          currentRoute == '/group' ? 'active' : '',
        ]"
        @click="toRoute('/group')"
      >
        <span>群聊</span>
      ...
      <div
        :class="[
          'navbar-icon navbar-collect',
          currentRoute == '/collect' ? 'active' : '',
        ]"
        @click="toRoute('/collect')"
      >
        <span>收藏</span>
      </div>
      <div
        v-if="switchConfig.isOpenFriendCircle.value"
        :class="['navbar-icon navbar-circle', isFriendCircle ? 'active' : '']"
        @click="openCircle"
      >
        <span>朋友圈</span>
        <!-- <i class="li-icon-new notreadwid size-t c-red"></i> -->
        <span v-if="notReadCircleNoticeNum > 0" class="notread notreadwid">{{
          notReadCircleNoticeNum
        }}</span>
        <span
          v-else-if="lastCircle && lastCircle.readflag != 1"
          class="notread_dot notreadwid"
        ></span>
      </div>
    </div>
    <!-- 其他 -->
    <div class="nav-footer" @click.stop="stopProp">
      <!-- <span class="icon_collect cursor" @click="collectShow"></span> -->
      <i
        :class="['footer-icon navbar-setting', moreshow ? 'active' : '']"
        @click="showMoreOper"
      ></i>
      <ul class="more-oper" v-show="moreshow">
        <li
          class="more-item"
        ...
        >
          ...
          />
          <span>{{
            currUser.phonebindflag == 1
              ? "修改手机"
              : currUser.phonebindflag == 2
              ? "绑定手机"
              : ""
          }}</span>
        </li>
        <li class="more-item" @click="showUpdatePwd">
          <i class="iconfont iconPassword"></i>
          <span>修改密码</span>
        </li>
        <li class="more-item" @click="showLogOut">
    ...
    <!-- 收藏列表 -->
    <div class="collect_list" v-show="collectShowFlag">
    ...
    ></CurrInfo>
    <!-- 修改密码 -->
    <UpdatePwd :pwdshow="pwdshow" @hideUpdatePwd="hideUpdatePwd"></UpdatePwd>
    <!-- 退出登录 -->
    <LogOut :outshow.sync="outshow"></LogOut>
    <!-- 绑定手机号 -->
    <Dialog v-show="bindMobileShow && switchConfig.isOpenVerifyCode.value == 1">
      <div class="bindMobile-modelbody modelbody">
        <div class="bindMobile-header">
          <p>绑定手机号</p>
          <p>
            根据国家有关法律法规要求，使用互联网服务需进行账号实名，请绑定实名手机号。
          </p>
        </div>
        <el-form :model="bindMobile.form" class="user-form">
          ...
          <el-form-item prop="mobileCode">
            <div class="input-row">
              ...
              <p
                ...
                @click.stop="
                  mobileTipsFlag ? '' : getVerify(bindMobile.form.loginName)
                "
              >
                {{ mobileTips }}
              </p>
          ...
          <div class="bindMobile-button-group">
            <p class="bindMobile-primarybtn cursor" @click="hideRegDialog">
              取消
            </p>
            <input
      ...
      <div class="updateMobile-modelbody modelbody">
        <div class="updateMobile-header">
          ...
          <div class="step-list">
            <ul class="step-ul">
              <li
                :style="{ background: oldMobileShow ? '#30D2B5' : '#F1F1F1' }"
              ></li>
              <li
                :style="{
                  background:
                    newsMobileShow && !mobileSucShow ? '#30D2B5' : '#F1F1F1',
                }"
              ></li>
              <li
                :style="{ background: mobileSucShow ? '#30D2B5' : '#F1F1F1' }"
              ></li>
            </ul>
          </div>
          <p v-show="oldMobileShow" class="updateMobile-Tips">
            短信验证码将发送至当前手机号：{{ mobileEncryption(currUser.phone) }}
          </p>
        </div>
        <i
          ...
          size="14px"
        ></i>
        <el-form :model="updateMobile.form" class="user-form">
          <el-form-item prop="mobileCode" v-show="oldMobileShow">
            <div class="input-row">
              <img
                class="iconfont"
                src="@/assets/imgs/login/check_icon.png"
          ...
          <div class="bot-group" v-show="oldMobileShow">
            <span>{{ errorMsg }}</span>
          </div>
          <div class="updateMobile-button-group" v-show="oldMobileShow">
            <input
          ...
          <el-form-item prop="loginName" v-show="newsMobileShow">
            <div class="input-row">
              <img
                class="iconfont"
                src="@/assets/imgs/login/mobile_icon.png"
          ...
          <el-form-item prop="mobileCode" v-show="newsMobileShow">
            <div class="input-row">
              <img
                class="iconfont"
                src="@/assets/imgs/login/check_icon.png"
              ...
              <p
                :class="[mobileTipsFlag ? 'countDown' : '', 'get-mobileCode']"
                @click.stop="
                  mobileTipsFlag ? '' : getVerify(updateMobile.form.loginName)
                "
              >
                {{ mobileTips }}
              </p>
          ...
          <el-form-item prop="passWord" v-show="newsMobileShow">
            <div class="input-row">
              <i class="iconfont iconttubiao_password"></i>
              <el-input
                :class="['user-input', seepwd ? '' : 'inputPsd']"
          ...
          <div class="bot-group" v-show="newsMobileShow">
            <span>{{ errorMsg }}</span>
          </div>
          <div class="updateMobile-button-group" v-show="newsMobileShow">
            <p class="cursor updateMobile-primarybtn" @click="updateMobileClik">
              提交
            </p>
          </div>
        </el-form>
    ...
    <Dialog v-show="mobileSucShow">
      <div class="modelbody">
        ...
        <div class="button-group">
          <p class="cursor primarybtn" @click="hideRegDialog">返回登录</p>
        </div>
      </div>
    </Dialog>
    <!-- 第三方登录绑定手机号提示弹窗 -->
    <Otherlogin v-show="othershow" :ortherLogin="ortherLogin"></Otherlogin>
    <!-- 个人信息二维码 -->
    <qrCode
    ...
    <drag-modal-vue
      v-model="isFriendCircle"
      :title="titles[circleType]"
      :width="610"
      :mask="false"
      :maskClosable="false"
    >
      <div class="circle_tools auto row-b-c" slot="close">
        <div class="row_c size-t" v-show="circleType == 0">
          <el-popover
            v-if="!circleMid"
            :value="circle_notices_show"
            placement="bottom"
            width="400"
            trigger="click"
          >
            <FriendCircleMessage
              ref="FriendCircleMessage"
              @seeCircle="seeCircle"
            />
            <div
              slot="reference"
              class="msg_icon pointer"
              @click.stop="openNotice"
            >
              <i class="el-icon-bell"></i>
              <div v-show="notReadCircleNoticeNum" class="dot row-c-c">
                {{ notReadCircleNoticeNum }}
              </div>
            </div>
          </el-popover>
          <i
            ...
            style="cursor: pointer"
            @click.stop="circleMid = ''"
          ></i>

          <i
            class="el-icon-refresh-right mg-l1"
            style="cursor: pointer"
            @click.stop="circleRfresh"
          ></i>
          <i
            class="el-icon-circle-plus-outline mg-l1"
            style="cursor: pointer"
            @click.stop="circleType = 1"
          ></i>
        </div>
        <div class="row_c size-t" v-show="circleType == 1">
          <i
            class="el-icon-arrow-left"
            style="cursor: pointer"
            @click.stop="circleType = 0"
          ></i>
        </div>
        <i
          class="iconfont iconIMweb_cancel_cancel size-max"
          style="cursor: pointer"
          @click.stop="isFriendCircle = false"
        ></i>
      </div>
      <FriendCircle
        ref="FriendCircle"
        ...
        @click.native="bodyClick"
        @close="mid = ''"
      />
      <FriendCirclePublich
        ref="FriendCirclePublich"
        v-show="circleType == 1"
...
import { chatcom, msgTips, user, circle } from "@/axios/path";
...
// import DragModal from "@/componentsThree/drag-modal-vue/src/components/modal.vue"

import FriendCircle from "@/components/circle/FriendCircle.vue";
import FriendCirclePublich from "@/components/circle/FriendCirclePublich.vue";
import FriendCircleMessage from "@/components/circle/FriendCircleMessage.vue";
...
export default {
  data() {
    return {
      ...
      logo: require("@/assets/imgs/common/im-logo.png"),
      codeValue: "", // 二维码内容
      user_avatar: "", //二维码头像
      qrmsg: "加我为好友",
      ...
      circle_notices_show: false,
      _circle_notices_show: false,
  ...
  computed: {
    ...mapState({
      ...
      switchConfig: (state) => state.CommonInfo.switchConfig, //前端功能开关配置
      lastCircle: (state) => state.Ws.lastCircle, //最后一条朋友圈，用于已读未读
      notReadCircleNoticeNum: (state) => state.Ws.notReadCircleNoticeNum, //朋友圈未读消息数量
    }),
    currentRoute() {
  ...
  mounted() {
    ...
    if (this.currUser.phonebindflag === 2) {
      if (this.currUser.thirdbindflag === 1) {
        // 绑定第三方，但是没有绑定手机号的弹出弹窗
        this.othershow = true;
      } else {
        // 目前只有邮箱账号，未来开放登录账号注册调整逻辑
  ...
  methods: {
    ...mapMutations([
      "setChatOn",
      "localReadCircle",
      "setNotReadCircleNoticeNum",
    ]),
    ...mapActions(["getNotReadNoticeNum"]),
    msgTips,
    /* 路由跳转 */
    toRoute(path) {
      this.$router.push(path);
      if (
        this.currUser.thirdbindflag === 1 &&
        this.currUser.phonebindflag === 2
      ) {
        // 绑定第三方，但是没有绑定手机号的弹出弹窗
        this.othershow = true;
      }
    ...
    async sendMsg() {
      ...
      let res = await chatcom.chatActChat(postdata);
      document.documentElement.click();
      if (res.ok) {
    ...
    verifySuccess(e) {
      ...
      if (this.bindMobileShow) {
        this.getMobileCode(
          1,
          this.bindMobile.form.loginName,
          captchaVerification
        ); //获取绑定手机号验证码
      } else if (this.updateMobileShow && this.oldMobileShow) {
        //输入原来手机号
        this.getMobileCode(5, this.currUser.phone, captchaVerification); //获取老手机验证码
      } else if (this.updateMobileShow && this.newsMobileShow) {
        //输入新手机号
        this.getMobileCode(
          7,
          this.updateMobile.form.loginName,
          captchaVerification
        ); //获取新手机验证码
      }
    },
    ...
    nextStepClick() {
      log("下一步");
      if (
        this.updateMobile.form.code == "" ||
        this.updateMobile.form.code == undefined
      ) {
    ...
    circleRfresh() {
      this.circleType = 0;
      this.$refs.FriendCircle.loadList("refresh", true);
    },
    publishSuccess() {
      this.circleType = 0;
      this.$refs.FriendCircle.loadList("refresh", false);
    },
    bodyClick() {
      this.circle_notices_show = false;
      this._circle_notices_show = false;

      console.log("bodyClick111", document.querySelector('.drag-modal-vue-modal'));
    },

    seeResource(e) {
      if (e.type == 2) {
        this.imgClick(e.data.imgs, e.data.index);
      } else if (e.type == 3) {
        this.videoClick(e.data);
      }
      console.log(e);
    },
    /* 消息中的图片点击事件 */
    imgClick(list, index = 0) {
      this.imgList = list;
      console.log(this.imgList);
      var timer = setTimeout(() => {
        var viewer = new Viewer(
          document.getElementById("view-container-circle"),
          {
            hidden: function () {
              viewer.destroy();
            },
    ...
    openCircle() {
      this.isFriendCircle = !this.isFriendCircle;
      if (this.isFriendCircle) {
        this.localReadCircle();
      }
    },
    openNotice() {
      this._circle_notices_show = !this._circle_notices_show;
      if (this._circle_notices_show) {
        this.$refs.FriendCircleMessage.loadList("refresh");
        circle.readCircleNotice().then((res) => {
          if (res.ok) {
            this.setNotReadCircleNoticeNum(0);
          }
        });
      }
    },
    seeCircle(item) {
      console.log(item);
      this.circleMid = item.moments.id;
    },
...
<style lang='less'>
.drag-modal-vue-modal-warp {
  position: static !important;
  z-index: 100 !important;
}
.drag-modal-vue-modal .drag-modal-vue-main {
  padding: 0 !important;
}
.drag-modal-vue-header {
  position: relative;
  justify-content: center !important;
}
.drag-modal-vue-title {
}
.drag-modal-vue-footer {
  padding: 0 !important;
}
