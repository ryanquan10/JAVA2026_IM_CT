<template>
<div class="search-container" :style="{height:show?'100%':'auto'}">
    <div class="search-in" v-show="!show">
        <div class="search-ipt">
            <i class="iconfont iconIMweb_search"></i>
            <input type="text" readonly placeholder="搜索" @click="showSearchList"/>
        </div>
        <i class="iconfont iconIMweb_add addicon" @click.stop="showMoreOper"></i>
        <ul class="more-oper" v-show="moreshow">
            <span class="popper__arrow"></span>
            <li class="more-item" @click="showCreateGroup">
                <i class="iconfont iconIMweb_group"></i>
                <span>创建群聊</span>
            </li>
            <li class="more-item" @click="showAddFriend">
    ...
    <div class="searchcontent" v-show="show">
        <div class="search-in">
            <div class="search-ipt">
                <i class="iconfont iconIMweb_search"></i>
                <input type="text" autocomplete="off" placeholder="搜索"  @input="searchFriends" v-model="searchVal" id="list-search">
        ...
        <div class="trans-list" id="search-mail-list">
            <div v-show="fList.length>0">
                ...
                <ul>
                    <li class="trans-row" v-for="item in fList" :key="item.uid" @click="searchLiClick(1,item.uid)">
            ...
            <div v-show="gList.length>0">
                ...
                <ul>
                    <li  class="trans-row" v-for="item in gList" :key="item.groupid" @click="searchLiClick(2,item.groupid)">
                        <el-image  :src="item.avatar" class="row-avatar">
                            <div slot="error" class="image-slot">
                                <img src="~@/assets/imgs/common/avatar.jpg" class="error-img"/>
                            </div>
    ...
    <Dialog v-show="addfriend">
        <div class="modelbody addfriend">
            <div class="title">
                <label>添加好友</label>
                <i class="iconfont iconIMweb_cancel_cancel" @click="cancleAdd"></i>
            </div>
            <div class="user-search">
                <div class="search-content">
                    <i class="iconfont iconIMweb_search"></i>
                    <input type="text" class="search-input" placeholder="用户ID/手机号" v-model="searchValue" @keyup.enter="searchFriend">
                </div>
                <button class="primarybtn"  @click="searchFriend">查找</button>
            </div>
            <div class="searchUserList" id="searchUserList">
                <ul  v-show="searchResult" id="wgkd">
                    ...
                    <li class="search-row"  v-else v-for="v in searchData" :key="v.id">
                        ...
                        <span class="apply-add" @click="userApply(v.id)">加好友</span>
                    </li>
                </ul>
            </div>
        </div>
    ...
    <Dialog v-show="creategroup">
            <div class="modelbody transcontainer">
                <p class="title">
                    ...
                    <i class="iconfont iconIMweb_cancel_cancel" @click="cancleCreate"></i>
                </p>
                <div class="trans-body">
                    <div class="trans-left">
                        <div class="fillheight">
                            <div class="search-content">
                                <i class="iconfont iconIMweb_search"></i>
                                <input type="text" placeholder="搜索" class="search-input" @input="friendQuerylist" v-model="pcardsearch">
                            </div>
                            <div class="trans-list" id="search-friend-list">
                                <ul>
                                    ...
                                    <div v-for="v in myFriendFilter" :key="v.index">
                                        ...
                                        <li class="trans-row"  v-for="item in v.data" :key="item.uid" @click="friendClick(item.uid)">
                                            <el-image class="row-avatar" :src="item.avatar">
                                                <div slot="error" class="image-slot">
                                                    <img src="~@/assets/imgs/common/avatar.jpg" class="error-img"/>
                                                </div>
                                            </el-image>
                                            <span class="row-name" v-html="brightenKeyword(item.remarkname||item.nick,pcardsearch)">
                                            </span>
                                            <span class="tm-checkbox" >
                                                <input type="checkbox"  :value="item.uid" v-model="choosedArr">  
                                            </span>
                                        </li>
                                    </div>
                                </ul>
                    ...
                    <div class="trans-right-list">
                        <p  class="choose-title">
                            ...
                            <button class="primarybtn" :disabled="choosedArr.length>0?false:true" @click="showSureCreate">创建群聊</button>
                        </p>
                        <div id="create-choosed-list" class="trans-choosed-list">
                            <ul>
                                <li class="trans-row"  v-for="choosed in choosedList" :key="choosed.uid">
        ...
        <div class="modelbody surecreate">
            ...
            <div class="create-content">
                <div class="create-row">
                    ...
                    <input type="text" class="create-name" maxlength="30" :placeholder="createname" v-model="groupname"/>
                </div>
                <div class="create-row">
                    <label class="create-label">群聊简介：</label>
                    <div class="areacontainer">
                        <textarea maxlength="500" v-model="groupintro" placeholder="请填写群简介"></textarea>
                        <p class="num-count">{{groupintro.length}}/500</p>
                    </div>
                </div>
            </div>
            <div class="button-group">
                <button class="primarybtn default" @click="closeCreate">取消</button>
                <button class="primarybtn" :disabled="loading" @click="wxSureCreate">创建</button>
            </div>
        </div>
    </Dialog>
</div>
        ...
        wxSureCreate(){
            ...
            group.createGroup(postdata).then(async res=>{
                if(res.ok){
                    ...
                    this.cancleCreate();
                    this.loading=false;
                    let actres=await chatcom.chatActChat({groupid:res.data.id});
