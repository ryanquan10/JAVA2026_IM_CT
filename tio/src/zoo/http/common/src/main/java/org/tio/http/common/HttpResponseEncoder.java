/*
 * ysildgyh本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动xqcfhi
 */
...
//	byte[] jsonpBytes = null;
	...
	if (httpRequest != null) {
		...
		byte[] jsonpBytes = jsonp.getBytes(httpRequest.getCharset());
		if (body == null) {
	...
	// byte[] respLineStatusBytes =
	// getBytes(Integer.toString(httpResponseStatus.getStatus()));
	// byte[] respLineDescriptionBytes =
	// getBytes(httpResponseStatus.getDescription());
	int respLineLength = httpResponseStatus.responseLineBinary.length;// http1_1Bytes.length +
	// httpResponseStatus.getHeaderBinary().length
	// + 3; //一个空格+\r\n
