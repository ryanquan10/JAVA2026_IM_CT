...
const GroupInfo={
    ...
    methods:{
        ...
        async getGroupInfo(){
            ...
            infoData.group.notice=infoData.group.notice||'';
      ...
      /**设置群公告滚动显示 */
      modifyNoticeFlag(noticeRoll){
        console.log(noticeRoll);
        let groupid = this.groupid
        let data={
            noticeRoll,
            groupid
        }
        var tips = noticeRoll==1?'已开启群公告滚动':'已关闭群公告滚动'
        group.modifyNoticeFlag(data).then(res=>{
            if(res.ok){
                this.groupInfo.noticeRoll = noticeRoll;
                store.commit("setGetGroupInfo", this.groupInfo);
                msgTips(tips)
            }else{
                msgTips(res.msg)
            }
      ...
      modifyGroupCardFlag(useCard){
        ...
        var tips = useCard==1?'已开启成员使用群名片':'已关闭成员使用群名片'
        group.modifyGroupCardFlag(data).then(res=>{
            if(res.ok){
                this.groupInfo.useCard = useCard;
                store.commit("setGetGroupInfo", this.groupInfo);
                msgTips(tips)
            }else{
                msgTips(res.msg)
      ...
      /**设置群成员消息是否能够撤回 */
      modifyMsgBackFlag(msgBack){
        let groupid = this.groupid
        let data={
            msgBack,
            groupid
        }
        var tips = msgBack==1?'已开启成员消息撤回':'已关闭成员消息撤回'
        group.modifyMsgBackFlag(data).then(res=>{
            if(res.ok){
                this.groupInfo.msgBack = msgBack;
                store.commit("setGetGroupInfo", this.groupInfo);
                msgTips(tips)
            }else{
                msgTips(res.msg)
      ...
      /**设置群成员对于普通用户是否可见 */
      modifyGroupMemberVisible(status){
        let groupid = this.groupid
        let data={
            groupid,
            isOpen: status,
        ...
        var tips = status==1?'已开启成员成员展示':'已关闭成员展示'
        group.modifyGroupSwitch(data).then(res=>{
            if(res.ok){
                this.groupInfo.isOpenMemberNum = status;
                store.commit("setGetGroupInfo", this.groupInfo);
                msgTips(tips)
            }else{
                msgTips(res.msg)
