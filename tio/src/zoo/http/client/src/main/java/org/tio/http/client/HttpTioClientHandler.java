/*
 * dmaqcqbi本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动ojptafalgd
 */
...
/**
 * 
 * @author tanyaowu 2018年7月8日 上午11:12:21
 */
public class HttpTioClientHandler implements TioClientHandler {
	...
	ClientHttpResponse response = HttpResponseDecoder.decode(buffer, limit, position, readableLength,
		channelContext);
	...
	if (packet.getByteCount() != null) {
	    HttpClientStarter.receivedBytes.addAndGet(packet.getByteCount());
	    HttpClientStarter.receivedStageBytes.addAndGet(packet.getByteCount());
	}
	long bs = HttpClientStarter.receivedBytes.get();
	long stageBs = HttpClientStarter.receivedStageBytes.get();
	...
	if (c % HttpClientStarter.stepCount == 0) {
	    ...
	    // System.out.println("已经完成请求数：" + StrUtil.fillBefore(c + "", '0', 10) + ",
	    // 本次耗时：" + stageIv + "ms, 总耗时：" + iv + "ms, " + "本次平均每秒处理请求数：" + (1000 *
	    // (stageC / stageIv)));
