    ...
    <!-- 消息右键操作框 -->
        ...
        <!-- 消息右键 -->
            <div v-show="cmenutype=='msg'">
                ...
                <li @click="dealMessage(9)" v-show="contextmenu.data.grouprole==3||contextmenu.data.grouprole==1||curruid==contextmenu.data.uid">撤回</li>
                <!-- 普通文本/图片消息右键显示复制||contextmenu.data.ct==6图片复制未完善 -->
                <li  v-show="contextmenu.data.ct==1" data-clipboard-action="copy"  class="copymessage" @click="contextmenu.data.ct==1?copyLink():contextmenu.data.ct==6?copyImg():''" >复制</li>
                <!-- 非名片|音视频|语音消息信息右键显示转发 -->
                <li @click="showTransMessage(1)" v-show="contextmenu.data.ct!=4&&contextmenu.data.ct!=9&&contextmenu.data.ct!=10&&contextmenu.data.ct!=11">转发</li>
                <!-- 另存为 图片/视频/文件 -->
                <li @click="saveAs"  v-show="contextmenu.data.ct==6||contextmenu.data.ct==5||contextmenu.data.ct==3">另存为</li>
                <!-- 信息右键删除 -->
            ...
            <div v-show="cmenutype=='avatar'&&contextmenu.data.isInGroup==1">
              ...
              <!-- 群用户的权限 -->
                <li  v-show="isFriend==1" @click="sendChatMsg">发消息</li>
                <li v-show="isFriend==2&&(contextmenu.data.friendflag&&contextmenu.data.friendflag==1)||isFriend==2&&(contextmenu.data.selfGrouprole==1||contextmenu.data.selfGrouprole==3)" @click="applyFriend">加好友</li>
                <!-- 群聊头像右键-@功能 -->
                <li @click="rightRemind" class="overell" v-show="contextmenu.data.isInGroup==1">@{{contextmenu.data.remindnick}}</li>
                <!-- 管理员只有群主能操作 -->
                <li @click="setManager(3)" v-show="contextmenu.data.selfGrouprole==1&&isforbidden.grouprole==2">设置管理员</li>
                <li @click="setManager(2)" v-show="contextmenu.data.selfGrouprole==1&&isforbidden.grant==1&&isforbidden.grouprole==3">取消管理员</li>
                <!-- 头像右键-禁言 grouprole 1群主,2成员,3管理员,grant 权限字段：1：有权限禁言；2：无权限 flag （禁言标识：1：时长禁言；2：否；3：长久禁用 管理员不能对群主进行禁言/取消禁言、移除群聊-->
                <li @click="prohibitSpeak" v-show="curruid!=contextmenu.data.uid&&(isforbidden.grant==1||isforbidden.rolegrant==1)&&isforbidden.flag==2&&(isforbidden.grouprole==2||isforbidden.grouprole=='undefind')">禁言</li>
                <li @click="relieveSpeak" v-show="curruid!=contextmenu.data.uid&&(isforbidden.grant==1||isforbidden.rolegrant==1)&&isforbidden.flag!=2&&(isforbidden.grouprole==2||isforbidden.grouprole=='undefind')">取消禁言</li>
                <!-- 移出群聊 判断是群主或者是管理员，但是管理员不能对群主操作，不能当前用户，操作对象在群内-->
                <li @click="kickGroup" v-show="contextmenu.data.isInGroup==1&&isforbidden.kickgrant==1&&(isforbidden.grouprole==2||isforbidden.grouprole=='undefind')">移出群聊</li>
            </div>
            <!-- 群聊头像右键 -->
            <div v-show="cmenutype=='gavatar'">
                <!--selfGrouprole 自己的角色权限，grouprole 群用户的权限 -->
                <li  v-show="isFriend==1" @click="sendChatMsg">发消息</li>
                <li v-show="isFriend==2&&(groupInfo.friendflag&&groupInfo.friendflag==1)||isFriend==2&&(contextmenu.data.selfGrouprole==1||contextmenu.data.selfGrouprole==3)" @click="applyFriend">加好友</li>
                <li v-show="isFriend==1"  @click="showTransMessage(2)">发名片</li>
                <!-- 管理员 -->
                <li @click="setManager(3)" v-show="contextmenu.data.selfGrouprole==1&&contextmenu.data.grouprole==2">设置管理员</li>
                <li @click="setManager(2)" v-show="contextmenu.data.selfGrouprole==1&&contextmenu.data.grouprole==3">取消管理员</li>
                <!-- 群聊头像右键-禁言 grouprole 1群主,2成员, forbiddenflag（禁言标识：1：时长禁言；2：否；3：长久禁用） 管理员不能对群主进行禁言/取消禁言、移除群聊-->
                <li @click="prohibitSpeak" v-show="(contextmenu.data.selfGrouprole==1||contextmenu.data.selfGrouprole==3)&&contextmenu.data.grouprole==2&&curruid!=contextmenu.data.uid&&contextmenu.data.forbiddenflag==2">禁言</li>
                <li @click="relieveSpeak" v-show="(contextmenu.data.selfGrouprole==1||contextmenu.data.selfGrouprole==3)&&contextmenu.data.grouprole==2&&curruid!=contextmenu.data.uid&&contextmenu.data.forbiddenflag!==2">取消禁言</li>
                <!-- 移出群聊 -->
                <li @click="kickGroup" v-show="isforbidden.kickgrant==1&&contextmenu.data.grouprole==2">移出群聊</li>
            </div>
        </ul>
        <!-- 删除消息 -->
        <Dialog v-show="delshow">
            <div class="modelbody">
                ...
                <div class="button-group">
                    <button class="primarybtn default" @click="cancleDel">取消</button>
                    <button class="primarybtn" @click="dealMessage(1)" :disabled="loading">确定</button>
                </div>
            </div>
        </Dialog>
        <!-- 消息转发 -->
        <TransMsg :transShow.sync="transShow" :transType="transType" :transData="contextmenu.data"></TransMsg>
        <!--群聊 确认移除选中的成员 -->
        <Dialog v-show="kickshow">
            <div class="modelbody">
                <p class="singletitle">确定将该用户移出群聊？</p>
                <div class="button-group">
                    <button class="primarybtn default" @click="cancleKick">取消</button>
                    <button class="primarybtn" :disabled="loading" @click="sureDelGroupMemb">删除</button>
                </div>
            </div>
        </Dialog>
        <!-- 群聊 禁言弹窗 -->
        <Dialog v-show="prohibitShow">
            <div class="modelbody prohibit-modelbody">
                ...
                <div>
                  <div  class="m-prohibit-list" v-for="(item,index) in prohibitList" :key="index" @click="rohibitIsSelect(item,index)">
                    <span>{{item.title}}</span>
                    <p class="m-checkbox">
                        <input type="checkbox" name="" id="" v-model="item.isCheck">
                    </p>
                  </div>
                </div>
                <div class="button-group">
                    <button class="primarybtn default" @click="prohibitShow = false">取消</button>
                    <button class="primarybtn" :disabled="loading" @click="sureForbidden">确定</button>
                </div>
            </div>
        </Dialog>
            <!-- 取消禁言 -->
        <Dialog v-show="relieveProhibit">
          <div class="modelbody">
            ...
            <div class="button-group">
              <button class="primarybtn default" @click="relieveProhibit = false">
                取消
              </button>
              <button class="primarybtn" @click="surerRelieveForbidden" :disabled="loading">
                确定
              </button>
            </div>
          </div>
        ...
        <Dialog v-show="withdrawShow">
          <div class="modelbody">
            ...
            <div class="button-group">
              <button class="primarybtn default" @click="withdrawShow = false">
                取消
              </button>
              <button class="primarybtn" @click="surerWithdraw" :disabled="loading">
                确定
              </button>
            </div>
          </div>
...
import {chatcom,group,friend,msgTips} from '@/axios/path';
...
export default {
    ...
    methods:{
        ...
        dealMessage(oper){
            ...
            chatcom.msgOper(ptdata).then(res=>{
                if(res.ok){
                    this.delshow=false;
                }else{
                    msgTips(res.msg);
