/*
 * wzfibyqohqmfp本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动vxcgxyjolnbvc
 */
...
/**
 * @author tanyaowu 2017年8月18日 下午5:47:00
 */
public class HttpGzipUtils {
	...
	if (request != null && request.getIsSupportGzip()) {
	    ...
	    if (request != null) {
		log.warn("{}, 不支持gzip, {}", request.getClientIp(),
			request.getHeader(HttpConst.RequestHeaderKey.User_Agent));
	    } else {
