<template>
  <div class="chatlist">
          ...
          <div class="chat-col-right">
            <p class="chat-info-top">
              ...
              <span
                ...
                >{{ item.notreadcount > 99 ? "..." : item.notreadcount }}</span
              >
            </p>
            <p class="chat-info-bottom">
              <span class="chat-last-msg">
                ...
                <span v-show="item.atreadflag == 2" class="notread"
                  >[有人@你]</span
                >
    ...
    >
      ...
      <!-- 会话列表-消息免打扰 -->
      <li @click="modifyGroupPush(contextmenu.data.msgfreeflag == 1 ? 2 : 1)">
        {{ contextmenu.data.msgfreeflag == 1 ? "取消免打扰" : "消息免打扰" }}
      </li>
      <!-- 会话列表的群聊右键-修改群名称 -->
      <li
...
export default {
  ...
  methods: {
    ...
    chatColClick(item) {
      ...
      console.log(store.state.Ws.allNotRead);
      if (store.state.Ws.allNotRead == 0) {
