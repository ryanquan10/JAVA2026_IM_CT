    ...
    <div class="commonright container">
        ...
        <div class="contentpad">
            <el-table height="700" :data="data.list" v-loading="data.loading"
                ...
                <el-table-column label="所属管理员" prop="mnick"></el-table-column>
                <el-table-column label="用户ID" prop="uid"></el-table-column>
                <el-table-column label="昵称" prop="unick"></el-table-column>
        ...
        <el-dialog :visible.sync="dialog.visible" :close-on-click-modal="false" class="tmdialog"
            ...
            <el-form :model="dialog.form" :rules="dialog.rules" :label-width="$protovar.fmlabwidth" ref="form"
                ...
                <div class="tmcol">
                    <el-form-item label="邀请码" prop="invitecode" maxlength="30">
                        <el-input class="name" v-model="dialog.form.invitecode" name="invitecode"></el-input>
                    </el-form-item>
                </div>
...
export default {
    ...
    methods: {
        ...
        getData(item) {
            ...
            console.log(this.curruser.rids)
            console.log(this.curruser.id)
            if (!this.curruser.rids.split(',').includes('1')) {
                // 非超级管理员
                ptdata.uid = this.curruser.id;
            }

            mguser.listInviteUser(ptdata).then((res) => {
                if (res.ok) {
                    let data = res.data;
                    if (data) {
