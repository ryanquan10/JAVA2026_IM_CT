/*
 * xsevcndwvehx本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动pvbwnocydxtfbk
 */
...
public abstract class CacheUtils {
	...
	String lockKey = cache.getCacheName() + "_^_$_" + cacheKey;
	try {
     ...
     * @param cacheKey             请业务侧保证cacheKey的唯一性，建议的做法是由prefix +
     *                             key组成，譬如"user.124578"，其中user就是prefix，124578就是key
     * @param putTempToCacheIfNull 当FirsthandCreater获取不到对象时，是否使用临时对象，以防缓存攻击。true:可以防止缓存null攻击
     * @param firsthandCreater
     * 
     * @return
	...
	if (caffeineCache == null) {
		...
		if (caffeineCache == null) {
		    caffeineCache = CaffeineRedisCache.register(redisson, cacheName, timeToLiveSeconds,
			    timeToIdleSeconds);
		}
	    }
	}
