/*
 * gbomizaxkcovmb本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动nmdltxsfn
 */
...
/**
 * 
 * @author tanyaowu 2017年4月15日 下午12:13:19
 */
public class TokenPathAccessStats {
	...
	this(statPathFilter, DefaultTokenGetter.me, currUseridGetter, tioConfig, tokenPathAccessStatListener,
		durations);
    }
     ...
     * @param durations
     */
    public TokenPathAccessStats(StatPathFilter statPathFilter, TokenGetter tokenGetter,
	    CurrUseridGetter currUseridGetter, TioConfig tioConfig,
	    TokenPathAccessStatListener tokenPathAccessStatListener, Long[] durations) {
	...
	if (tokenAccessStat == null && forceCreate) {
		...
		if (tokenAccessStat == null) {
		    tokenAccessStat = new TokenAccessStat(duration, token, ip, uid);// new MapWithLock<String,
		    // TokenPathAccessStat>(new
		    // HashMap<>());//new
		    // TokenPathAccessStat(duration,
		    // token, path);
