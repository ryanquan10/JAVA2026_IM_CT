...
let tioCache = {
  ...
  set: function(cacheName, key, value, options) {
    ...
    // console.log('tioCacheSet', this.get(cacheName, key));
  },
  get: function(cacheName, key) {
    var ret = wsCache.get(cacheName + tioCache.splitStr + key)
    // console.log('tioCacheGet', cacheName, key, ret);
    return ret
  },
...
// 判断两个时间是否超过预定的时间长度
function checkTimeout(b_time, c_time) {
  if (!b_time) {
    return true;
  }
  let flag = false;
  b_time = handleTimeToArray(b_time);
  c_time = handleTimeToArray(c_time);
  if (c_time[0] > b_time[0]) {
    flag = true;
  } else if (c_time[1] > b_time[1]) {
    flag = true;
  } else if (c_time[2] - b_time[2] >= 5) {
    flag = true;
  }
  return flag;
}
function handleTimeToArray(time) {
  let t = time.split(" ");
  let t1 = t[0].split("-").join("");
  let t2 = t[1].split(":");
  let arr = [+t1, +t2[0], +t2[1]];
  return arr;
}
export {
  resUrl,
