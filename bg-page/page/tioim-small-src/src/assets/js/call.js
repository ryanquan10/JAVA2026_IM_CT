...
import {audio} from '@/assets/js/ws/callAgora.js'
...
function setLocalSrcObj(bodyObj, stream) {
    let isOpenAgora = state.CommonInfo.switchConfig.isOpenAgora.value;  // // 是否已启用声网配置
    if(isOpenAgora){
...
function wxCallClear() {
    ...
    if(state.Call.agoraRTC){
        store.dispatch('stopAgoraRTC'); 
    }
}
