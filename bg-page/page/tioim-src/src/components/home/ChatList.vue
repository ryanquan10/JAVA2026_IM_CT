<template>
  <div class="chatlist">
    <div id="tioim-chat-list" class="tioim-chat-list">
      <ul>
        ...
        <li :class="[
          ...
          ]" v-for="item in chatList" :key="item.id" @click="chatColClick(item)" @contextmenu.prevent="chatContextMenu($event, item)">
          ...
          <div class="chat-col-right">
            <p class="chat-info-top">
              ...
              <span class="chat-notread" v-if="item.notreadcount > 0&&item.msgfreeflag!=1">{{
                item.notreadcount > 99 ? "..." : item.notreadcount
              }}</span>
            </p>
            <p class="chat-info-bottom">
              <span class="chat-last-msg">
                <span class="msgfreeNotread" v-if="item.notreadcount > 0&&item.msgfreeflag==1">[{{item.notreadcount > 99 ? "..." : item.notreadcount}}条消息]</span>
                ...
                <!-- 如果为群聊，显示 昵称:内容 chatmode（1为私聊，2为群聊）sysflag（最后一条消息系统标识：1：是系统消息；2：是正常消息） -->
                    ...
                    (item.chatmode == 2 &&item.sysflag&&item.sysflag == 2 &&item.ct != 12 &&item.msgtype ==1&&item.msgresume&&item.ct != 13) || (item.ct &&
                      ...
                      ? '分享一个链接'
                      : item.msgresume
                  " class="chat-last-bot"></span>
              </span>
              <span class="chat-last-time" v-show="!(callShow && callInfo.id == item.bizid)">{{ getShowTime(item.sendtime, "chat") }}</span>
              <i :class="[
                  'iconfont',
                  'rtcicon',
                  calltype == 1 ? 'iconvoicecall' : 'iconVideocall',
                ]" v-if="callShow && callInfo.id == item.bizid"></i>
            </p>
          </div>
          <div class="chat-msgfree" v-show="item.msgfreeflag==1">
            <p class="icon_msgfree_notread" v-show="item.notreadcount > 0"></p>
            <img class="icon_msgfree" src="~@/assets/imgs/common/icon_msgfree.png" alt="">
          </div>
    ...
    <!-- 会话右键操作框 -->
    <ul v-show="contextmenushow" class="contextmenu-ul" :style="{ top: contextmenu.top, left: contextmenu.left }">
      <!-- 会话列表右键-置顶 -->
      ...
      <!-- 会话列表的群聊右键-修改群名称 -->
      <li @click="editGroupName(contextmenu.data)" v-show="contextmenu.data.bizrole == 1">
        修改群名称
      </li>
    </ul>
    <!-- 修改群名称 -->
    ...
    <Dialog v-show="delshow">
      <div class="modelbody delchat">
        <div class="maintitle">
          ...
          <p class="tmcheckbox">
            <input type="checkbox" v-model="delcheck" />
            同时删除聊天记录
          </p>
        </div>
        <div class="button-group">
          <button class="primarybtn default" @click="cancleDel">取消</button>
          <button class="primarybtn" @click="chatOper(delcheck ? 1 : 11)" :disabled="loading">
            确定
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    chatOper(oper) {
      ...
      chatcom.chatOper(postdata).then((res) => {
        if (res.ok) {
    ...
    modifyGroupPush(freeflag){
        ...
        chatcom.msgfreeflag(data).then(res=>{
            if(res.ok){
