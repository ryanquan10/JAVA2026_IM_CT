<template>
  <div class="more-body">
    ...
    <div class="morecontent" v-show="groupmore">
      ...
      <div class="group_tab">
        <ul class="group_tab_list cursor">
          <li
            :class="['group_tab_item', tabIndex == index ? 'tab_select' : '']"
      ...
      <div v-show="tabIndex === 0" class="groupcontainer">
        <div class="group_info">
          <div class="groupInfo-avatar">
            ...
            <span
              v-show="groupUser.grouprole == 1 || groupUser.grouprole == 3"
              class="hoverblack"
          ...
          <p class="groupInfo_name cursor info-row" @click="editGroupName">
            {{ groupInfo_name }}
            <i
              class="iconfont iconttubiao_bianji edit"
              v-show="groupUser.grouprole == 1 || groupUser.grouprole == 3"
              @click="editGroupName"
            ></i>
          </p>
          <div class="icon_share cursor" @click="shareGroup(groupInfo)">
            <p></p>
          </div>
        ...
        <div class="groupInfo_content">
          <p
            v-if="groupInfo.isOpenMemberNum == 1 || groupUser.grouprole != 2"
            class="info-row"
          >
            <label class="info-label">群人数</label>
            <span class="info-content" @click="goMemberTab"
              >{{ member.totalRow + Number(groupInfo.vnum)}}人</span>
          </p>
          <p class="info-row">
            ...
            <span
              ...
              @click="
                showModel(
                  groupUser.grouprole == 1 || groupUser.grouprole == 3
                    ? 'editintro'
                    : 'groupintro'
                )
              "
              >{{ groupInfo.intro || "暂无群简介" }}</span
            >
            <i
              class="iconfont iconttubiao_bianji edit"
              v-if="groupUser.grouprole == 1 || groupUser.grouprole == 3"
              @click="showModel('editintro')"
            ></i>
          </p>
          <p class="info-row">
            <label class="info-label">群签到</label>
            <span class="info-content" @click="openGroupSign">{{
              "去签到"
            }}</span>
            <!-- <i
              class="iconfont iconttubiao_bianji edit"
          ...
          <p class="info-row">
            ...
            <i
              class="iconfont iconttubiao_bianji edit"
              v-if="groupUser.grouprole == 1 || groupUser.grouprole == 3"
              @click="showModel('editnotices')"
            ></i>
          </p>
          <p class="info-row">
            <label class="info-label">群主</label>
            <span class="info-content no-cursor">{{
              myRemarks[groupInfo.uid]
                ? myRemarks[groupInfo.uid]
                : groupInfo.nick
            }}</span>
          </p>
          <p class="info-row">
            ...
            <span class="info-content" @click="showModel('editnick')">{{
              groupUser.groupnick
            }}</span>
            <i
              class="iconfont iconttubiao_bianji edit"
              @click="showModel('editnick')"
            ></i>
          </p>
          <p class="info-row">
            ...
            <label class="tioim-switch" @click="changeInvit('freeflag')">
              <span
                :class="[
                  'switch-span',
                  groupUser.msgfreeflag == 1 ? 'checked' : '',
                ]"
              ></span>
            </label>
            <span class="apply-status">{{
              groupUser.msgfreeflag == 1 ? "免打扰" : "正常"
            }}</span>
          </p>
          <!-- <p class="info-row">
      ...
      >
          ...
          <div class="searchlist" id="searchlist">
            <ul>
              <li
                ...
                :key="item.uid"
                @click.stop="showCard($event, item.uid)"
              ...
              >
                ...
                <p
                  class="row-name"
                  v-html="
                    brightenKeyword(myRemarks[item.uid] || item.nick, searchVal)
                  "
                ></p>
                <p
                  v-if="switchConfig.groupShowRole.value"
                  :class="[
                    item.grouprole == 1 || item.selfGrouprole === 1
                      ? 'groupUser'
                      : item.grouprole == 3 || item.selfGrouprole == 3
                      ? 'groupManage'
                      : '',
                  ]"
                ></p>
        ...
        <div v-show="!searchshow" class="group-main-info">
          ...
          <div
            id="membercontent"
            :class="['membercontent', seemorememb ? 'minmember' : 'maxmember']"
          >
            <ul class="memberlist">
              <li
                class="memeber-col flex_column_between_Center"
                v-show="
                  groupUser.grouprole == 1 ||
                  groupUser.grouprole == 3 ||
                  groupInfo.applyflag == 1
                "
              >
              ...
              >
                ...
                <el-image
                  class="user-avatar"
                  :src="v.avatar"
                  @click.stop="showCard($event, v.uid)"
                ...
                <p
                  v-if="switchConfig.groupShowRole.value"
                  :class="[
                    v.grouprole == 1 || v.selfGrouprole === 1
                      ? 'groupUser'
                      : v.grouprole == 3 || v.selfGrouprole == 3
                      ? 'groupManage'
                      : '',
                  ]"
                ></p>
                <p class="user-name">{{ myRemarks[v.uid] || v.nick }}</p>
              </li>
            </ul>
          </div>
          <p
            ...
            v-show="
              seemorememb &&
              (groupUser.grouprole == 1
                ? groupInfo.joinnum > 13
                : groupInfo.joinnum > 14)
            "
            @click="seeMoreMember(false)"
          >
            查看更多<i class="iconfont iconttubiao_jiantou"></i>
          </p>
          <p
            class="moredown moreup"
            v-show="!seemorememb"
            @click="seeMoreMember(true)"
          >
            收起<i class="iconfont iconttubiao_shangla"></i>
          </p>
        </div>
      </div>
      <!-- 群管理 -->
      <div v-show="tabIndex === 2" class="groupcontainer">
        <div class="group-main-info group_manage">
          ...
          <p class="info-row">
            ...
            <label
              class="tioim-switch"
              @click="changeInvit('modifyFriendFlag')"
            >
              <span
                :class="[
                  'switch-span',
                  groupInfo.friendflag == 1 ? 'checked' : '',
                ]"
              ></span>
            </label>
          </p>
          ...
          <p class="info-row">
            <label class="info-label">公告滚动显示</label>
            <label
              class="tioim-switch"
              @click="changeInvit('modifyNoticeFlag')"
            >
              <span
                :class="[
                  'switch-span',
                  groupInfo.noticeRoll == 1 ? 'checked' : '',
                ]"
              ></span>
            </label>
          </p>
          <p class="info-row">
            <label class="info-label">允许分享群名片</label>
            <label
              class="tioim-switch"
              @click="changeInvit('modifyGroupCardFlag')"
            >
              <span
                :class="[
                  'switch-span',
                  groupInfo.useCard == 1 ? 'checked' : '',
                ]"
              ></span>
            </label>
          </p>
          <p class="info-row">
            <label class="info-label">成员消息撤回</label>
            <label
              class="tioim-switch"
              @click="changeInvit('modifyMsgBackFlag')"
            >
              <span
                :class="[
                  'switch-span',
                  groupInfo.msgBack == 1 ? 'checked' : '',
                ]"
              ></span>
            </label>
          </p>
          <p class="info-row">
            <label class="info-label">成员展示</label>
            <label
              class="tioim-switch"
              @click="changeInvit('modifyGroupMemberVisible')"
            >
              <span
                :class="[
                  'switch-span',
                  groupInfo.isOpenMemberNum == 1 ? 'checked' : '',
                ]"
              ></span>
            </label>
          </p>
          <p class="info-row">
            <label class="info-label auto">群成员标签</label>
            <span class="info-content" @click="showModel('editlabel')">{{
              groupInfo.groupmemberlabel
            }}</span>
            <i
              class="iconfont iconttubiao_bianji edit"
              @click="showModel('editlabel')"
            ></i>
          </p>
          <div class="groupmanage_content">
            <p class="info-row">
              ...
              <label class="tioim-switch" @click="allForbidden">
                <span
                  :class="[
                    'switch-span',
                    groupInfo.forbiddenflag == 1 ? 'checked' : '',
                  ]"
                ></span>
              </label>
            </p>
            ...
            <div class="groupmanage_groupsBtn">
              <button class="cursor" @click="showModel('outgroup')">
                退出群
              </button>
              <button
                class="cursor"
                @click="showModel('delgroup')"
                v-show="groupUser.grouprole == 1"
              >
                解散群
              </button>
              <button
                class="cursor"
                @click="showModel('transgroup')"
                v-show="groupUser.grouprole == 1"
              >
                转让群
              </button>
    ...
    <GroupModel
      ...
      :extra="{ groupNotices: groupNotices, groupSignInfo }"
      @getGroupMembers="getGroupMembers"
      @changeType="modelChangeType"
      @getGroupInfo="getGroupInfo12()"
    ></GroupModel>
    <!-- 修改群名称 -->
    <GroupName
      :show.sync="editname"
      ...
      @saveSuccessName="saveSuccessName"
    ></GroupName>
    <!-- 修改图片 -->
    <GroupAvatar
        ...
        <div class="searchUserList" id="searchUserList">
          <ul v-show="searchResult">
            ...
            <li class="search-row" v-else v-for="v in searchData" :key="v.id">
              ...
              <span
                class="forbidden_duration cursor"
                @click="removeForbidden(v)"
                >{{ SecondToDate(v.forbiddenduration) }}</span
              >
              <img
                class="icon_remove cursor"
                src="~@/assets/imgs/group/icon_remove.png"
                alt=""
                @click="removeForbidden(v)"
              />
            </li>
    ...
    <Dialog v-show="forbiddenList" style="z-index: 1001">
      <div class="modelbody remove-modelbody">
        <div class="singletitle">确认将{{ removeInfo.nick }}移除禁言名单？</div>
        <div class="button-group">
          <button class="primarybtn default" @click="forbiddenList = false">
            取消
          </button>
          <button class="primarybtn" @click="remvoeForbidden">确定</button>
        </div>
      </div>
    </Dialog>
    <!-- 分享群聊 -->
...
export default {
  ...
  data() {
    return {
      ...
      infoshow: false,
      infotype: "",
      isGroupSign: false, // 群签到显示状态
      groupSignInfo: null, // 当前用户在这个群的签到记录
    };
  },
  watch: {
    async groupmore(nv) {
      if (nv) {
        ...
        if (this.groupUser.grouprole == 1 || this.groupUser.grouprole == 3) {
          ...
          if (this.groupInfo.isOpenMemberNum != 1) {
            this.tabList.splice(1, 1);
          }
          this.isGroupLeader = false;
        }
        this.groupInfo_name =
          this.groupInfo.name.length > 20
            ? this.groupInfo.name.substring(0, 20) + "..."
            : this.groupInfo.name;
      } else {
        this.tabList = [];
        this.tabIndex = 0;
  ...
  computed: {
    ...mapState({
      ...
      myRemarks: (state) => state.User.myRemarks, //个人备注集合
  ...
  methods: {
    ...
    changeInvit(type) {
      ...
      } else if (type == "modifyFriendFlag") {
        // 群内互加好友
        if (this.groupInfo.friendflag == 1) {
          //关闭
          this.modifyFriendFlag(2);
        } else {
          this.modifyFriendFlag(1);
        }
      } else if (type == "modifyNoticeFlag") {
        // 更改公告显示方式
        if (this.groupInfo.noticeRoll == 1) {
          //关闭
          this.modifyNoticeFlag(0);
        } else {
          this.modifyNoticeFlag(1);
        }
      } else if (type == "modifyGroupCardFlag") {
        // 修改允许分享群名片
        if (this.groupInfo.useCard == 1) {
          //关闭
          this.modifyGroupCardFlag(0);
        } else {
          this.modifyGroupCardFlag(1);
        }
      } else if (type == "modifyMsgBackFlag") {
        // 群成员消息撤回
        if (this.groupInfo.msgBack == 1) {
          //关闭
          this.modifyMsgBackFlag(0);
        } else {
          this.modifyMsgBackFlag(1);
        }
      } else if (type == "modifyGroupMemberVisible") {
        // 群成员展示
        if (this.groupInfo.isOpenMemberNum == 1) {
          //关闭
          this.modifyGroupMemberVisible(2);
        } else {
          this.modifyGroupMemberVisible(1);
        }
      }
    },
    /* 查看更多群成员 */
          ...
          this.$nextTick(() => {
            defineScroll($("#searchlist"), "", {
              whileScrolling: function () {
                if (
                  this.mcs.topPct == 95 &&
                  _this.smember.pagenum < _this.smember.totalPage
                ) {
            ...
            this.$nextTick(() => {
              defineScroll($("#searchUserList"), "", {
                whileScrolling: function () {
                  if (
                    this.mcs.topPct == 95 &&
                    _this.search.pagenum < _this.search.totalPage
                  ) {
    ...
    afterDelGroup(uid) {
      ...
      //左侧群聊的人数
      this.applyThis.$refs.grouplist.groupList.find(
        (item) => item.groupid == this.groupid
      ).joinnum = this.groupInfo.joinnum;
    },
    /**展示用户名片 */
    async showCard(e, uid) {
      ...
      if (this.groupInfo.friendflag == 2) {
        if (
          isfriend == 2 &&
          (this.groupUser.grouprole == 1 || this.groupUser.grouprole == 3)
        ) {
    ...
    modelChangeType(e) {
      if (e.type == "publishNotice") {
        this.showModel("editnotice");
      } else if (e.type == "backList") {
        this.showModel("editnotices");
      }
    },
    getGroupInfo12() {
      ...
      group.getchatGroupInfo({ groupid, userflag: 1 }).then((res) => {
        if (res.ok) {
          ...
          this.groupNotices = infoData.group.groupNotices;
          console.log("groupNotices", infoData.group.groupNotices);
    ...
    openGroupSign() {
      ...
      group.groupUserSignInfo(params).then((res) => {
        if (res.ok) {
          let data = res.data;
          let signInfoMap = {};
          let time_curr = getFormatTime(null, "-");
          data.signInfo.map((item) => {
            let key = item.signtime.slice(0, 10);
            // 如果列表中有今天的时间，将当日签到状态改为true
            console.log(time_curr, key);
            if (time_curr == key) {
              data.isSign = true;
            }
            signInfoMap[key] = true;
          });
          data.signInfoMap = signInfoMap;
          this.groupSignInfo = data;
          this.isGroupSign = true;
      this.showModel("isGroupSign");
          console.log(this.groupSignInfo);
        }
      });
      
    },
    closeGroupSign() {
      this.isGroupSign = false;
    },
  },
};
</script>
