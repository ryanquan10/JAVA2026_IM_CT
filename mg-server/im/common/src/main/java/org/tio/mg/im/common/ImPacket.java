...
import org.tio.mg.im.common.bs.base.Base;
...
public class ImPacket extends Packet {
	...
	public WsResponseExt toWs() {
		...
		synchronized (this) {
				...
				if (bodyBytes.length > ImConst.SIZE_FOR_COMPRESS) {
					try {
						...
						if (gzipedbody.length < bodyBytes.length) {
							...
							//							ByteBuffer byteBuffer = ByteBuffer.allocate(2 + bodyBytes.length);
							//							byteBuffer.putChar('x');
							//							byteBuffer.put(bodyBytes);

							//							byte[] newBodyBytes = new byte[bodyBytes.length + 2];
							//							
							//							System.arraycopy(BYTES_X, 0, newBodyBytes, 0, BYTES_X.length);//  byteBuffer.array();
							//							System.arraycopy(bodyBytes, 0, newBodyBytes, 2, bodyBytes.length);
							//							
			...
			wsPacket = wsResponse;
			return wsResponse;//WsResponse.fromText(text, Const.CHARSET);
		}
