/*
 * psdiqupwipxzwq本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动mukjvnrgdrfan
 */
/**
 * 
 */
...
public class CaffeineCache extends AbsCache {
	...
	if (caffeineCache == null && !skipNull) {
	    log.error("cacheName[{}]还没注册，请初始化时调用：{}.register(cacheName, timeToLiveSeconds, timeToIdleSeconds)",
		    cacheName, CaffeineCache.class.getSimpleName());
	}
	...
	if (caffeineCache == null) {
		...
		if (caffeineCache == null) {
		    ...
		    LoadingCache<String, Serializable> temporaryLoadingCache = CaffeineUtils.createLoadingCache(
			    cacheName, 10L, (Long) null, initialCapacity, temporaryMaximumSize, recordStats,
			    removalListener);
    ...
    private CaffeineCache(String cacheName, LoadingCache<String, Serializable> loadingCache,
