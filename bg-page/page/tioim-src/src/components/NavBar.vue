<template>
  <div class="navbar">
    <div class="nav-top user">
      <el-image class="user-avatar" :src="currUser.avatar" @click.stop="showUserCard">
        <div slot="error" class="image-slot">
          <img src="~@/assets/imgs/common/avatar.jpg" class="error-img" @click.stop="showUserCard" />
        </div>
      </el-image>
      <!-- 用户信息 -->
      <div class="userinfo" @click.stop="stopProp" v-show="show">
        <div class="baseinfo">
          ...
          <div class="info-avatar">
            ...
            <img src="~@/assets/imgs/common/upload.png" class="hover-icon" @click="showUpdateInfo(2)" />
          </div>
        ...
        <div class="info-group-btn">
          <button class="primarybtn" @click="showUpdateInfo(1)">修改信息</button>
    ...
    <div class="nav-center">
      <div :class="['navbar-icon navbar-chat',currentRoute=='/home'?'active':'']" @click="toRoute('/home')">
        <span>聊天</span>
        <span v-if="allNotRead!=0" class="notread notreadwid">{{allNotRead}}</span>
      </div>
      <div :class="['navbar-icon navbar-friends',currentRoute=='/friend'?'active':'']" @click="toRoute('/friend')">
        <span>好友</span>
        <span v-if="noReadApply!=0" class="notread notreadwid">{{noReadApply}}</span>
      </div>
      <div :class="['navbar-icon navbar-groups',currentRoute=='/group'?'active':'']" @click="toRoute('/group')">
        <span>群聊</span>
      </div>
    </div>
    <!-- 其他 -->
    <div class="nav-footer" @click.stop="stopProp">
      ...
      <i :class="['iconfont iconIMweb_more',moreshow?'active':'']" @click="showMoreOper"></i>
      <ul class="more-oper" v-show="moreshow">
        <li class="more-item" @click.stop="updateShow(currUser.phonebindflag)">
          <img class="ws3fuuv5" src="@/assets/imgs/login/mobile_icon.png" alt="" srcset="">
    ...
    <div class="collect_list" v-show="collectShowFlag">
      <div class="collect-header">
        <span>我的收藏</span>
        <i class="iconfont iconIMweb_cancel_cancel cancel cursor" @click="collectShowFlag = false" size="14px"></i>
      </div>
      <div class="collect-content">
        <ul class="tab-list">
          <li :class="['tab-item cursor',tabIndex==index?'tab_select':'']" v-for="(item,index) in tabList" :key="index" @click="switchTab(index)">
              {{item.title}}
              <p></p>
          </li>
    ...
    <!-- 修改用户信息 -->
    <CurrInfo :infoshow.sync="infoshow" :userinfo="userinfo" :type="infotype"></CurrInfo>
    <!-- 修改密码 -->
        ...
        <el-form :model="bindMobile.form" class="user-form">
          <el-form-item prop="loginName">
            <div class="input-row">
              ...
              <el-input class="user-input" type="text" maxlength='11' v-model="bindMobile.form.loginName" placeholder="请输入您的手机号" @input="mobileInput"></el-input>
            </div>
          </el-form-item>
          <el-form-item prop="mobileCode">
            <div class="input-row">
              <img class="iconfont" src="@/assets/imgs/login/check_icon.png" alt="" srcset="">
              <el-input class="user-input" type="text" v-model="bindMobile.form.code" placeholder="请输入您的验证码" @input="codeInput"></el-input>
              <p style="cursor:pointer;" :class="[mobileTipsFlag?'countDown':'','get-mobileCode']" @click.stop="mobileTipsFlag?'':getVerify(bindMobile.form.loginName)">{{mobileTips}}</p>
            </div>
          </el-form-item>
          <el-form-item prop="passWord">
            <div class="input-row">
              ...
              <el-input :class="['user-input',seepwd ?'':'inputPsd']"   
                   v-model="bindMobile.form.passWord" placeholder="请输入当前账号密码" autoComplete="off"></el-input>
              <i :class="[
                    'iconfont cursor icon-pwd',
                    seepwd ? 'iconttubiao_display' : 'iconttubiao_hide',
                  ]" @click="seepwd=!seepwd"></i>
            </div>
          </el-form-item>
          <div class="bot-group">
            <span>{{errorMsg}}</span>
          ...
          <div class="bindMobile-button-group">
            <p class="bindMobile-primarybtn cursor" @click="hideRegDialog">取消</p>
            <input class="primarybtn bindMobile-primarybtn cursor" :disabled="(checkMobileFlag&&!checkSmsFlag)||(!checkMobileFlag&&checkSmsFlag)||(!checkMobileFlag&&!checkMobileFlag)" @click="bindMobileClik" readonly="readonly" value="确定"/>
    ...
    <Dialog v-show="updateMobileShow">
      <div class="updateMobile-modelbody modelbody">
        <div class="updateMobile-header">
          ...
          <p v-show="oldMobileShow" class="updateMobile-Tips">短信验证码将发送至当前手机号：{{mobileEncryption(currUser.phone)}}</p>
        </div>
        <i class="iconfont iconIMweb_cancel_cancel cursor" @click="updateMobileShow = !updateMobileShow" size="14px"></i>
        <el-form :model="updateMobile.form" class="user-form">
          <el-form-item prop="mobileCode" v-show="oldMobileShow">
            <div class="input-row">
              <img class="iconfont" src="@/assets/imgs/login/check_icon.png" alt="" srcset="">
              <el-input class="user-input" type="text" v-model="updateMobile.form.code" placeholder="请输入您的验证码" @input="codeInput" ></el-input>
              <p :class="[mobileTipsFlag?'countDown':'','get-mobileCode']" @click.stop="mobileTipsFlag?'':getVerify(currUser.phone)">{{mobileTips}}</p>
            </div>
          </el-form-item>
          ...
          <div class="updateMobile-button-group" v-show="oldMobileShow">
            <input class="primarybtn updateMobile-primarybtn cursor" :disabled="!checkSmsFlag" @click="nextStepClick" readonly="readonly" value="下一步"
          />
          </div>
          <el-form-item prop="loginName" v-show="newsMobileShow">
            <div class="input-row">
              <img class="iconfont" src="@/assets/imgs/login/mobile_icon.png" alt="" srcset="">
              <el-input class="user-input" type="text" maxlength='11' v-model="updateMobile.form.loginName" placeholder="请输入您的手机号" @input="mobileInput"></el-input>
            </div>
          </el-form-item>
          <el-form-item prop="mobileCode" v-show="newsMobileShow">
            <div class="input-row">
              <img class="iconfont" src="@/assets/imgs/login/check_icon.png" alt="" srcset="">
              <el-input class="user-input" type="number" v-model="updateMobile.form.newCode" placeholder="请输入您的验证码"  autoComplete="new-password" @input="codeInput"></el-input>
              <p :class="[mobileTipsFlag?'countDown':'','get-mobileCode']" @click.stop="mobileTipsFlag?'':getVerify(updateMobile.form.loginName)">{{mobileTips}}</p>
            </div>
          </el-form-item>
          <el-form-item prop="passWord" v-show="newsMobileShow">
            <div class="input-row">
              <i class="iconfont iconttubiao_password"></i>
              <el-input :class="['user-input',seepwd ?'':'inputPsd']"   v-model="updateMobile.form.passWord" placeholder="请输入当前账号密码" autoComplete="off"></el-input>
              <i :class="['iconfont cursor icon-pwd',seepwd ? 'iconttubiao_display' : 'iconttubiao_hide',]" @click="seepwd = !seepwd"></i>
            </div>
          </el-form-item>
          ...
          <div class="updateMobile-button-group" v-show="newsMobileShow">
            <p class="cursor updateMobile-primarybtn" @click="updateMobileClik">提交</p>
          </div>
        </el-form>
      </div>
    </Dialog>
    ...
    <!-- 个人信息二维码 -->
    <qrCode :infoQrCode.sync="infoQrCode" :codeValue="codeValue" :user_avatar="user_avatar" :currUser="currUser" :qrmsg="qrmsg" @hideInfoQrCode="hideInfoQrCode"></qrCode>
  </div>
</template>
<script>
