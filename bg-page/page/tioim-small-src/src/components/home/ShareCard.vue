<template>
  <div class="card-container" @click.stop="stopProp">
            ...
            <p>
              ...
              >
                ...
                <span
                  class="flexauto"
                  v-html="
                    brightenKeyword(item.remarkname || item.nick, pcardsearch)
                  "
                >
                </span>
              </li>
      ...
      <div class="modelbody sharebody">
        ...
        <div class="share-content">
          <div class="card-info">
            ...
            <p class="share-tips">
              {{ sharetype == 1 ? "个人名片" : "群名片" }}
            </p>
          </div>
          <p class="to-title">
            将名片分享给{{ isGroup ? "该群聊" : "该用户" }}
          </p>
        ...
        <div class="button-group">
          <button class="primarybtn default" @click="cancleToShare">
            取消
          </button>
          <button class="primarybtn" @click="sureSendCard" :disabled="loading">
            发送名片
          </button>
        </div>
      </div>
    ...
    <div class="remindContent send-card" v-show="gcarshow">
      ...
      <div id="groupListCard" class="remindbody cardlist">
        <ul class="remindfriends card-content">
          ...
          <li
            class="remind-col flexbox cursor"
            v-for="item in groupCardFilter"
            :key="item.groupid"
            @click="showCard(item, 2)"
          >
            <el-image class="remind-col_img" :src="item.avatar">
              <div slot="error" class="image-slot">
                <img src="~@/assets/imgs/common/avatar.jpg" class="error-img" />
            ...
            <span
              class="flexauto"
              v-html="brightenKeyword(item.name, gcardsearch)"
            >
            </span>
          </li>
        </ul>
    ...
    <!-- 拖拽发送文件 -->
    <div class="remindContent send-card send_file" v-show="fileShow">
      <p class="remind-title">
        <span>发送给：</span>
        <i
          class="iconfont iconIMweb_cancel_cancel closeicon"
      ...
      <div class="send-file-main">
        <div class="user_info row_c">
          <el-image class="avatar" :src="chatinfo.avatar">
            <div slot="error" class="image-slot">
              <img src="~@/assets/imgs/common/avatar.jpg" class="error-img" />
            </div>
          </el-image>
          <div class="row_c_l_name">
            <span class="row_c_l_name_name">{{ chatinfo.name }}</span>
          </div>
        </div>
        <div class="line"></div>
        <ul class="file_list">
          <li
            class="file_item row-b-c"
            v-for="(file, ix) in files"
            :key="file.id"
            @click="previewFile(ix)"
          >
            <div class="file-flex row_c">
              <img
                v-if="file.simpType == 'docx'"
              ...
              />
              <img
                v-else-if="file.simpType == 'xls' || file.simpType == 'xlsx'"
                src="~@/assets/imgs/msglist/xls.png"
                alt=""
              />
              <img
                v-else-if="file.simpType == 'mp4'"
                src="~@/assets/imgs/msglist/mp4.png"
                alt=""
              />
              <img
                v-else-if="file.simpType == 'mp3'"
                src="~@/assets/imgs/msglist/mp3.png"
                alt=""
              />
              <img
                v-else-if="file.simpType == 'pdf'"
                src="~@/assets/imgs/msglist/pdf.png"
                alt=""
              />
              <img
                v-else-if="file.simpType == 'ppt' || file.simpType == 'pptx'"
                src="~@/assets/imgs/msglist/ppt.png"
                alt=""
              />
              <img
                v-else-if="file.simpType == 'zip'"
                src="~@/assets/imgs/msglist/zip.png"
                alt=""
              />
              <img
                v-else-if="file.simpType == 'apk'"
                src="~@/assets/imgs/msglist/apk.png"
                alt=""
              />
              <img
                v-else-if="file.simpType == 'txt'"
                src="~@/assets/imgs/msglist/txt.png"
                alt=""
              />
              <img v-else src="~@/assets/imgs/msglist/notRecogn.png" alt="" />
              <div class="file_info">
                <div class="file_name" v-html="strRep(file.name)"></div>
                <div class="file_size">
                  {{ formatFileSize(file.size) }}
                </div>
              </div>
            </div>
            <div class="del_btn row-c-c" @click.stop="delFile(ix)">
              <i class="li-icon-garbage"></i>
            </div>
          </li>
        </ul>
      </div>
      <div class="button-group file_btm">
        <button class="primarybtn" @click="sendFiles" :disabled="loading">
          发送
        </button>
        <button class="primarybtn default" @click="closeDialog">取消</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  props: ["pcarshow", "gcarshow", "fileShow", "files"],
  data() {
    return {
  ...
  methods: {
    ...
    async getMyGroupList() {
      ...
      if (mailData) {
        ...
        list = list.filter((item) => {
          return (
            item.grouprole == 1 || item.grouprole == 3 || item.use_card == 1
          );
        });
      }
      return list;
    ...
    previewFile(ix) {
      ...
      let file = files[ix];
      let obj;
      if (file.genericsType == "image") {
        obj = {
          ix,
          type: "image",
          list: files.filter((item) => item.genericsType == "image").map(item=>{
            return {
              imgsrc: item.localUrl,
              mid: item.id
            }
          }),
        };
        this.$emit("preview", obj);
      } else if (file.genericsType == "video") {
        obj = {
          type: "video",
          video: {
