    ...
    <div class="contentpad">
      ...
      <template slot-scope="props">
        <el-form label-position="left" inline class="demo-table-expand">
          <el-form-item label="被投诉用户ID">
            <span>{{ props.row.reportedUserId }}</span>
          </el-form-item>
        ...
        <el-table-column label="被投诉用户" prop="reportedUserNick" width="120px"></el-table-column>
        <el-table-column label="被投诉用户状态" width="120px">
          <template slot-scope="scope">
            <span v-if="scope.row.reportedUserStatus == 1" class="tmopera">正常</span>
            <span v-if="scope.row.reportedUserStatus == 5" class="tmopera waring">已停用</span>
          </template>
        </el-table-column>
        <el-table-column label="投诉用户" prop="reportUserNick"></el-table-column>
        <el-table-column label="投诉内容" prop="reason" width="400px"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getData(item) {
      ...
      report.friendList(ptdata).then((res) => {
        if (res.ok) {
          let data = res.data;
          if (data) {
    ...
    handleReport(id, status){
      let ids = [id];
      let title = status == 1? '确认要将这个用户封号吗？' : '忽略这条消息?';
      this.$alert(title, "提示", {
        ...
        callback: (action) => {
          ...
          if (action == "confirm") {
            ...
            report.friendHandle(ptdata).then((res) => {
              if (res.ok) {
                successTips("操作成功");
                this.getData();
              } else {
