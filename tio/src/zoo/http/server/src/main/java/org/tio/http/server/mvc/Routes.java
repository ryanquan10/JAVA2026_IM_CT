/*
 * atvlsvcehycg本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动heiwxnaxs
 */
					...
					ClassUtil.scanPackage(pkg, new ClassScanAnnotationHandler(RequestPath.class) {
						...
						public void handlerAnnotation(Class<?> clazz) {
							try {
								...
								if (beanPath != null) {
									processBeanPath(clazz, bean, beanPath, classMapping);
								}		
								String[] beanPathArr = classMapping.valueList();
								if (cn.hutool.core.util.ArrayUtil.isNotEmpty(beanPathArr)) {
									for (String beanPath1 : beanPathArr) {
										processBeanPath(clazz, bean, beanPath1, classMapping);
									}
								}
...
//////////////////////
	...
	private void processBeanPath(Class<?> clazz, Object bean, String beanPath, RequestPath classMapping) {
		...
		c: for (Method method : methods) {
			...
			if (methodPath != null) { //这里只能是null，不要加上""
				processMethodPath(bean, beanPath, mapping, method, methodPath);
			}
			String[] methodPathArr = mapping.valueList();
			if (cn.hutool.core.util.ArrayUtil.isNotEmpty(methodPathArr)) {
				for (String methodPath1 : methodPathArr) {
					processMethodPath(bean, beanPath, mapping, method, methodPath1);
				}
			}
...
//			}
	...
	private void processMethodPath(Object bean, String beanPath, RequestPath mapping, Method method, String methodPath) {
		String completePath = beanPath + methodPath;
		Class<?>[] parameterTypes = method.getParameterTypes();
		try {
			Paranamer paranamer = new BytecodeReadingParanamer();
			...
			Method checkMethod = PATH_METHOD_MAP.get(completePath);
			if (checkMethod != null) {
				log.info("mapping[{}] already exists in method [{}]", completePath, checkMethod.getDeclaringClass() + "#" + checkMethod.getName());
				errorStr.append("mapping[" + completePath + "] already exists in method [" + checkMethod.getDeclaringClass() + "#"
				        + checkMethod.getName() + "]\r\n\r\n");
//				continue c;
				return;
