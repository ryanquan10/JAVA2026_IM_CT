  ...
  <div class="maincontent">
    ...
    <div class="chatinfo">
      <span class="chat-name">{{
        !isGroup ? chatinfo.name : chatinfo.name + "(" + chatinfo.joinnum + ")"
      }}</span>
      <div class="info-opera">
        <!-- 举报icon -->
        <span class="cursor" v-show="(isGroup && chatinfo.linkflag == 1)||!isGroup" @click="reportShow=true,isGroup?reportmsg='群聊':reportmsg='用户'"></span>
        <!-- 群聊头部右侧操作图标 -->
        <i class="iconfont iconttubiao_point" v-show="isGroup && chatinfo.linkflag == 1" @click.stop="chatGroupSet"></i>
        <!-- 私聊头部右侧操作图标 -->
        <i class="iconfont iconttubiao_point" v-show="!isGroup" @click.stop="chatPersonSet"></i>
      </div>
    </div>
    <!-- 消息列表 -->
    <div class="msgcontainer" id="msgcontainer">
      <ul>
        ...
        <div v-show="nextPage && !lastPage" class='msg-col msg-center-col msg-moretop' @click="getNextMsg">查看更多消息</div>
        <li :class="[
            !item.delwhere && item.singleshow
              ? item.type == 1 && item.ct !== 12
                ...
                : 'msg-hide'
              : 'msg-hide',
          ]" v-for="item in MessageList" :key="item.mid" :mid="item.mid">
          <!-- 如果为系统消息，且满足单通道显示-->
          ...
          <div v-if="item.type == 3 &&item.ct==13">
            <p>{{ getShowTime(item.t) }}</p>
            <p>{{ item.name}}想邀请好友加入群聊，<span class="cursor examine" @click="goExamine(item)">{{item.apply.status==1?'已确认':'去确认'}}</span></p>
          </div>
          <!-- 系统消息的红包提示 -->
          ...
          <div v-if="item.type == 4">
            ...
            <span class="sysmsg">{{ item.html
              }}<span class="sendapply" @click="sendApply">发送好友验证</span></span>
          </div>
          ...
          <template v-if="
              ((item.type == 1 && item.ct !== 12 && item.ct !== 13) ||
                ...
                !item.delwhere
            ">
            <el-image class="msg-avatar" :src="item.avatar" @contextmenu.prevent="chatContextMenu($event, item, 'avatar')" @click.stop="showCard($event, item.uid,item)">
              <div slot="error" class="image-slot">
                <img src="~@/assets/imgs/common/avatar.jpg" class="error-img" @contextmenu.prevent="chatContextMenu($event, item, 'avatar')" @click.stop="showCard($event, item.uid,item)" />
              </div>
            </el-image>
            <div class="msg-right" @contextmenu.prevent="chatContextMenu($event, item, 'msg')">
              <p class="msg-top">
              ...
              <div :class="['msg-bot',item.ct == 5 || item.ct == 6 || item.ct == 9? 'msg-bot-nbg': item.ct == 88? 'msg-bot-nbg not-max-width':'',]">
                <i v-if="item.touid != curruid" :class="['msgreadsta',item.readflag == 1 ? 'wxc_read' : 'wxc_notread',]">
                  {{item.readflag == 1? "已读": item.readflag == 2? "未读": ""}}</i>
                <!--  1、普通文本消息，2、超链接卡片消息，3、文件，4、音频，5、视频 ,6.图片 ,9.名片 ,10.视频通话, 11.音频通话, 12.红包-->
                <div v-if="item.ct == 1" :id="'copy' + item.mid" v-html="item.html"></div>
                <div v-else-if="item.ct == 3" class="file-flex">
                  ...
                  <img v-if="item.fc.ext == 'docx'" src="~@/assets/imgs/msglist/world.png" alt="" />
                  <img v-else-if="item.fc.ext == 'jpg' || item.fc.ext == 'png' || item.fc.ext == 'gif'" src="~@/assets/imgs/msglist/jpg.png" alt="" />
                  <img v-else-if="item.fc.ext == 'xls' || item.fc.ext == 'xlsx'" src="~@/assets/imgs/msglist/xls.png" alt="" />
                  <img v-else-if="item.fc.ext == 'mp4'" src="~@/assets/imgs/msglist/mp4.png" alt="" />
                  <img v-else-if="item.fc.ext == 'mp3'" src="~@/assets/imgs/msglist/mp3.png" alt="" />
                  <img v-else-if="item.fc.ext == 'pdf'" src="~@/assets/imgs/msglist/pdf.png" alt="" />
                  <img v-else-if="item.fc.ext == 'ppt' || item.fc.ext == 'pptx'" src="~@/assets/imgs/msglist/ppt.png" alt="" />
                  <img v-else-if="item.fc.ext == 'zip'" src="~@/assets/imgs/msglist/zip.png" alt="" />
                  <img v-else-if="item.fc.ext == 'apk'" src="~@/assets/imgs/msglist/apk.png" alt="" />
                  <img v-else-if="item.fc.ext == 'txt'" src="~@/assets/imgs/msglist/txt.png" alt="" />
                  <img v-else src="~@/assets/imgs/msglist/notRecogn.png" alt="" />
                </div>

                <div v-else-if="item.ct == 4" class="audiomsg" @click="playAudio(item)" :style="{ width: item.bodyData.width + 'px' }">
                  <span>{{ item.bodyData.seconds }}″</span>
                  <span v-html="item.html"></span>
                  <img src="~@/assets/imgs/home/ownvoice_stop.png" v-show="item.type == 1 && !item.bodyData.play" />
                  <img src="~@/assets/imgs/home/voice_stop.png" v-show="item.type == 2 && !item.bodyData.play" />
                  <img src="~@/assets/imgs/home/ownvoice.gif" v-show="item.type == 1 && item.bodyData.play" />
                  <img src="~@/assets/imgs/home/voice.gif" v-show="item.type == 2 && item.bodyData.play" />
                </div>
                <!-- 视频 -->
                <div v-else-if="item.ct == 5" class="videocol" @click="videoClick(item.bodyData)">
                  <el-image :src="item.bodyData.vcoverurl" fit="cover" class="el-image" :style="{
                ...
                <!-- 图片 -->
                <div v-else-if="item.ct == 6" class="imgcol cursor" :id="'copy' + item.mid">
                  <img :src="item.bodyData.coverurl" fit="cover" :id="'copyImg' + item.mid" class="el-image" @click="imgClick(item.mid)" :style="{
                ...
                <!-- 名片 -->
                <div v-else-if="item.ct == 9" class="cardbg" @click.stop="cardClick($event, item)">
                  <div class="cardtop">
                    <el-image :src="item.bodyData.bizavatar" fit="cover" class="el-image">
                      <div slot="error" class="image-slot">
                        <img src="~@/assets/imgs/common/avatar.jpg" class="error-img" />
                      </div>
                    </el-image>
                  ...
                  <div class="cardbot">
                    <img src="~@/assets/imgs/home/scard.png" v-show="item.bodyData.cardtype == 1" />
                    <img src="~@/assets/imgs/home/gcard.png" v-show="item.bodyData.cardtype == 2" />
                    {{ item.bodyData.cardtype == 2 ? "群名片" : "个人名片" }}
                  </div>
                </div>
                <!-- 音视频通话 10:视频通话；11:音频通话-->
                <div v-else-if="item.ct == 10 || item.ct == 11" class="callcol">
                  <img src="~@/assets/imgs/home/video.png" v-if="item.ct == 10" :class="[
                      'call-icon',
                      item.type == 1 ? 'right-icon rotate' : '',
                    ]" />
                  <img src="~@/assets/imgs/home/autio.png" v-if="item.ct == 11 && item.type == 2" class="call-icon" />
                  <img src="~@/assets/imgs/home/autiort.png" v-if="item.ct == 11 && item.type == 1" class="call-icon right-icon" />
                  ...
                  <div class="hyperlinks cursor" @click="openNews(item.temp.url)">
                    ...
                    <div class="hyperlinks-content">
                      ...
                      <el-image :src="item.temp.img" fit="cover" class="el-image">
                        <div slot="error" class="image-slot">
                          <img src="~@/assets/imgs/msglist/link_default_img.png" class="error-img" />
                        </div>
                      </el-image>
                    </div>
                  </div>
    ...
    <div class="chat-bottom" id="chat-bottom">
      <div class="chat-send-opera">
        <div class="chat-send-icon">
          <i title="选择表情" class="iconfont iconIMweb_expression" @click="IconColor('iconIMweb_expression')" @click.stop="chooseEmoji"></i>
          ...
          <i title="@" class="iconfont iconIMweb_" v-show="isGroup" @click="IconColor('iconIMweb_')" @click.stop="getRemindContent"></i>
          <i title="推荐好友" class="iconfont iconIMweb_grcard" @click="IconColor('iconIMweb_grcard')" @click.stop="shareFriend">
          </i>
          <i title="分享群聊" class="iconfont iconIMweb_qcard" @click="IconColor('iconIMweb_qcard')" @click.stop="shareGroup">
          </i>
          <i title="语音通话" class="iconfont iconvoicecall" v-if="!isGroup" @click="IconColor('iconvoicecall')" @click.stop="wxCallInvite(1)">
          </i>
          <i title="视频通话" class="iconfont iconVideocall" v-if="!isGroup" @click="IconColor('iconVideocall')" @click.stop="wxCallInvite(2)">
          </i>
        </div>
      </div>
      <!-- 输入聊天内容文本框 -->
      <div id="chat-editor" name="content" class="chat-editor" @keydown.enter.prevent="wxTextKey"  @paste="pasteSend"  contenteditable="true" @input="listenRemind" @blur="getSelecRange"></div>
      <div class="chat-send-bot">
        ...
        <button class="primarybtn" @click.stop="chatSendMessage">发送</button>
      </div>
      <!-- 表情弹框 -->
      <div class="tm-emoji-container" v-show="showEmoji">
        <div id="tm-emoji-body">
          <ul class="tm-emoji-body flexbox">
            <li v-for="(emoji, index) in emojiList" :key="index" @click="appendMessage" :alt="emoji.alt">
              <img :src="staticUrl + 'static/emoji/emoji/' + emoji.url" class="small-emoji" style="width:24px;" />
            </li>
          </ul>
        </div>
      ...
      <!-- 分享好友名片|群聊名 -->
      <ShareCard :pcarshow.sync="pcarshow" :gcarshow.sync="gcarshow"></ShareCard>
    </div>
    ...
    <Dialog v-show="video.show">
      <div class="modelbody videomodel">
        <p class="title">
          ...
          <i class="iconfont iconIMweb_cancel_cancel closeicon" @click="closeVideo"></i>
        </p>
        <video :src="video.url" id="tm-video" controls loop="loop" autoplay="autoplay"></video>
      </div>
    </Dialog>
    <!-- 确定加入群聊-弹框 -->
    ...
    <div id="remindContent" class="remindList" @click.stop="stopProp">
      <div class="remindContent" v-show="remindshow">
        <p class="remind-title">
          <span>选择需要提醒的人</span>
          <i class="iconfont iconIMweb_cancel_cancel closeicon" @click="hideRemind"></i>
        </p>
        <p class="tm-search-friend">
          <i class="iconfont iconIMweb_search"></i>
          <input type="text" autocomplete="off" placeholder="请输入好友名称" class="tm-search-input" id="remindsearch" v-model="remindsearch" @input="remindSearch" />
        </p>
        <div id="remindfriends" class="remindbody">
          <ul class="remindfriends">
            <li class="remind-col"  @click.stop="addReMind(atAll)">
              <img class="icon_all" src="~@/assets/imgs/msglist/@all.png" alt="">
              <span>提醒所有人</span>
            </li>
            <li class="remind-col" @click.stop="addReMind(item)" v-for="item in atMemList" :key="item.uid" v-show="curruid != item.uid">
    ...
    <!-- 消息右键操作框 -->
    <ContextMenu :show="contextmenushow" :contextmenu="contextmenu"  :groupInfo="groupInfo" :cmenutype="cmenutype" :isFriend="isFriend" :isforbidden="isforbidden" @rightRemind="rightRemind" @userApply="userApply"></ContextMenu>
    <!-- 个人名片 -->
    <UserCard :show.sync="cardshow" :userCard="userCard" ref="usercard"></UserCard>
    <!-- 群聊信息 -->
    <GroupMore ref="groupmore" :show.sync="groupmore"></GroupMore>
    <!-- 群聊相关弹框 -->
    <GroupModel ref="groupmodel" :groupInfo="commGroupInfo" :groupApplyInfo="groupApplyInfo" ></GroupModel>
    <!-- 举报弹窗 -->
    <Dialog v-show="reportShow">
      <div class="modelbody reportShow-model">
        ...
        <div class="areacontainer">
          <textarea maxlength="500" v-model="reportInput" placeholder="请填写举报投诉原因"></textarea>
          <p class="num-count">{{ reportInput.length }}/500</p>
        </div>
        <div class="button-group">
          <button class="primarybtn default" @click="reportShow=false">取消</button>
...
import {group,friend,msgTips,chatcom} from '@/axios/path';
export default {
  data() {
    return {
      ...
      defineScroll(
        ...
        scrollTopPx + "px",
        {
          whileScrolling:function() {
            if (this.mcs.draggerTop <= 30 &&this.mcs?.draggerTop>=5&& !_this.nextPage) {
              _this.nextPage=true
