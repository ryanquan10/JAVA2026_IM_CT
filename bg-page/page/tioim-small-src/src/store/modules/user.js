...
import {commonView,currUser, group} from '@/axios/path';
const state={
    currUser:null,//当前用户
    ...
    myRemarks: {}, // 个人对所有的好友或陌生人备注
    myRemarkState: false
};
const actions={
    /* 获取配置 */
    async getComView({state,commit}){
        console.log('before initCookie1', JSON.parse(JSON.stringify(document.cookie)));
        let viewData=await commonView();
        console.log('before initCookie2', JSON.parse(JSON.stringify(document.cookie)));
        let viewConfig;
        if(viewData.ok){
    ...
    async getCurrUser({state,commit}){
        ...
        if(res.ok){
            ...
            $("#tiotitle").html(data.nick); //将登陆账户设为标题
            state.currUser=data;
    ...
    // 获取个人对所有的好友或陌生人备注
    getRemarkList({state, commit}){
        group.getRemarkList().then((res)=>{
            if(res.ok){
                state.myRemarks = res.data
            }
        })
    },
...
const mutations={
    ...
    addMyRemarks(state, val){
        if(val.remarkname){
            state.myRemarks[val.uid] = val.remarkname;
        }else{
            delete state.myRemarks[val.uid];
        }
        state.myRemarkState = !state.myRemarkState;
        console.log('addMyRemarks done', state.myRemarks)
    }
};
export default {
    state,
