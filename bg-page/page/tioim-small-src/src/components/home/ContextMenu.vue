  ...
  <div>
    ...
    >
      ...
      <div v-show="cmenutype == 'msg'">
        ...
        <li
            @click="handleCallBack(contextmenu.data.ct)"
            v-show="contextmenu.data.ct == 10 || contextmenu.data.ct == 11"
        >
          回拨
        </li>
        ...
        <li
          @click="dealMessage(9)"
          v-show="
            contextmenu.data.grouprole == 3 ||
            contextmenu.data.grouprole == 1 ||
            (curruid == contextmenu.data.uid && contextmenu.data.msgBack != 0)
          "
        >
          撤回
        </li>
        ...
        <li
          ...
          class="copymessage"
          @click="
            contextmenu.data.ct == 1
              ? copyLink()
              : contextmenu.data.ct == 6
              ? copyImg()
              : ''
          "
        >
        ...
        <!-- 非名片|音视频|语音消息信息右键显示多选 -->
        <li
          @click="showManySelectMessage()"
          v-show="
            contextmenu.data.ct != 4 &&
            contextmenu.data.ct != 9 &&
            contextmenu.data.ct != 10 &&
            contextmenu.data.ct != 11 &&
            contextmenu.data.ct != 20
          "
        >
          多选
        </li>
        ...
        <li
          ...
          v-show="
            contextmenu.data.ct == 6 ||
            contextmenu.data.ct == 5 ||
            contextmenu.data.ct == 3
          "
        >
          另存为
        </li>
        <!-- 收藏 -->
        <li
          v-show="contextmenu.data.ct >= 1 && contextmenu.data.ct <= 6"
          @click="collectMsg(contextmenu.data)"
        >
          收藏
        </li>
        <!-- 引用 -->
        <li
          v-show="[9, 20].includes(contextmenu.data.ct) || (contextmenu.data.ct >= 1 && contextmenu.data.ct <= 6)"
          @click="quoteMsg(contextmenu.data)"
        >
          引用
        </li>
        <!-- 信息右键删除 -->
      ...
      <div v-show="cmenutype == 'avatar' && contextmenu.data.isInGroup == 1">
        ...
        <li
          v-show="
            (isFriend == 2 &&
              contextmenu.data.friendflag &&
              contextmenu.data.friendflag == 1) ||
            (isFriend == 2 &&
              (contextmenu.data.selfGrouprole == 1 ||
                contextmenu.data.selfGrouprole == 3))
          "
          @click="applyFriend"
        >
          加好友
        ...
        >
          @{{ contextmenu.data.remindnick }}
        </li>
        <!-- 管理员只有群主能操作 -->
        <li
          @click="setManager(3)"
          v-show="
            contextmenu.data.selfGrouprole == 1 && isforbidden.grouprole == 2
          "
        >
          设置管理员
        </li>
        ...
        <li
          @click="prohibitSpeak"
          v-show="
            curruid != contextmenu.data.uid &&
            (isforbidden.grant == 1 || isforbidden.rolegrant == 1) &&
            isforbidden.flag == 2 &&
            (isforbidden.grouprole == 2 || isforbidden.grouprole == undefind)
          "
        >
          禁言
        </li>
        ...
        <li
          @click="kickGroup"
          v-show="
            contextmenu.data.isInGroup == 1 &&
            isforbidden.kickgrant == 1 &&
            (isforbidden.grouprole == 2 || isforbidden.grouprole == undefind)
          "
        >
          移出群聊
      ...
      <div v-show="cmenutype == 'gavatar'">
        ...
        <li
          v-show="
            (isFriend == 2 &&
              groupInfo.friendflag &&
              groupInfo.friendflag == 1) ||
            (isFriend == 2 &&
              (contextmenu.data.selfGrouprole == 1 ||
                contextmenu.data.selfGrouprole == 3))
          "
        ...
        <li
          ...
          v-show="
            ...
            contextmenu.data.grouprole == 2 &&
            curruid != contextmenu.data.uid &&
            contextmenu.data.forbiddenflag == 2
          "
        >
          禁言
        </li>
      ...
      <div v-show="cmenutype == 'collect'">
        ...
        <li
          @click="saveAs"
          v-show="contextmenu.data.ct == 5 || contextmenu.data.ct == 3"
        >
          另存为
        </li>
        <!-- 信息右键删除 -->
    ...
    <Dialog v-show="prohibitShow">
      <div class="modelbody prohibit-modelbody">
        ...
        <div class="button-group">
          <button class="primarybtn default" @click="prohibitShow = false">
            取消
          </button>
          <button class="primarybtn" :disabled="loading" @click="sureForbidden">
            确定
          </button>
        </div>
      </div>
    ...
    <Dialog v-show="relieveProhibit">
      <div class="modelbody">
        <div class="singletitle">确定要取消禁言吗？</div>
        <div class="button-group">
    ...
    <Dialog v-show="withdrawShow">
      <div class="modelbody">
        <div class="singletitle">确定撤回该群聊消息？</div>
        <div class="button-group">
...
import { chatcom, group, friend, collect, msgTips } from "@/axios/path";
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      _groupInfo: (state) => state.Ws.groupInfo, //当前会话-是否为群聊
    }),
  },
  components: {
    TransMsg,
  ...
  methods: {
    ...
    /* 撤回|删除消息 1：删除；9：撤回 */
    dealMessage(oper) {
      if (this.cmenutype == "collect") {
        this.$emit("deleteCollect", this.contextmenu.data);
        this.delshow = false;
        return;
      }
      this.loading = true;
    ...
    collectMsg(data) {
      console.log(data);
      let fromId = this.bizId;
      if (!this.isGroup && this.contextmenu.is_right) {
        fromId = this.curruid;
      }
      let params = {
        fromType: this.isGroup ? 2 : 1,
        fromId: fromId,
        category: data.ct,
      };
      let { chatlinkid, ct, mid, c } = data;
      let _data = {
        chatlinkid,
        ct,
        mid,
      ...
      params.content = JSON.stringify(_data);
      collect.add(params).then((res) => {
        if (res.ok) {
          msgTips("内容已收藏");
        } else {
          msgTips(res.msg);
    ...
    showTransMessage(transType) {
      console.log("showTransMessage", this.contextmenu.data);
      this.transType = transType;
      this.transShow = true;
    },
    // 多选消息
    showManySelectMessage(){
       this.$emit("showManySelect", this.contextmenu.data);
    },
    /* 发消息 */
    ...
    setManager(grouprole) {
      ...
      var tips =
        grouprole == 3 ? `设置${nick}为管理员成功` : `已取消${nick}的管理员`;
    ...
    async getGroupInfo() {
      ...
      infoData.group.noticetime = infoData.group.noticetime
        ? infoData.group.noticetime.substring(0, 16)
        : "";
