<template>
  <div class="commonright container">
    ...
    <div class="contentpad">
      <el-card class="box-card" v-if="recommendCityList">
        ...
        <div class="city_tags">
          <el-tag
            class="city_tag"
            closable
            v-for="city in recommendCityList"
            :key="city.id"
            @close="changeCityRecommendStatus(city.id, 0)"
          >
            {{ city.city }}
          </el-tag>
        </div>
      </el-card>
      <el-card class="box-card" v-if="data.list.length > 0">
        <div slot="header" class="clearfix">
          <span>添加推荐城市</span>
        </div>
        <div>
        ...
        <el-button class="city_add_btn" type="primary" size="small" @click="addCityRecommend"
          >添加</el-button
        >
      </el-card>

      <!-- <div class="container">
        <p class="container_title">推荐城市</p>
...
export default {
  ...
  methods: {
    ...
    getData(item) {
      ...
      circleCenter.cityList(ptdata).then((res) => {
        if (res.ok) {
          let data = res.data;
          if (data) {
    ...
    getRecommendCityList() {
      circleCenter.recommendCityList().then((res) => {
        if (res.ok) {
          this.recommendCityList = res.data;
        } else {
          msgTips(res);
        }
      });
    ...
    // 设置/删除推荐城市
    changeCityRecommendStatus(cityId, status) {
      let params = { status, cityId };
      circleCenter.setCityRecommend(params).then((res) => {
        if (res.ok) {
          if (status == 1) {
            successTips("设置成功");
          } else {
    ...
    addCityRecommend(){
        let cityId = this.citySelected;
        this.changeCityRecommendStatus(cityId, 1);
    }
  },
};
</script>
