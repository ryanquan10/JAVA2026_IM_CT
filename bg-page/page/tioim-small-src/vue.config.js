...
module.exports = {
    publicPath: process.env.NODE_ENV === 'production' ? '/tioims' : '/', //vueConf.publicPath, // 根域上下文目录
    ...
    css: {
        loaderOptions: {
            less: {
                lessOptions: {
                    javascriptEnabled: true // 嵌套在 lessOptions 中
                }
            }
        }
    },
    ...
    devServer: {
        ...
        proxy: {
            "/mytio": {
                target: "http://localhost:6060",   // 本地模拟数据服务器
                // target: "https://api.bangban.pro",  // 生产环境模拟数据服务器
                //    target: "https://c-api.qmxtcq.cn",  // 生产环境模拟数据服务器
                changeOrigin: true,
                pathRewrite: {
                    "^/mytio": "" // 去掉接口地址中的api字符串,本地关闭
                },
                // cookieDomainRewrite: 'c-api.qmxtcq.cn'// 重写cookie 的domain,在localhost上工作时，cookie域必须设置为“”或NULL或FALSE
