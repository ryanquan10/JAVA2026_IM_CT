/*
 * nzigafeezpe本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动okkfficunaxwxh
 */
...
import org.tio.http.common.HeaderValue.EnumerableValue;
...
public class Resps {
	...
	if (StrUtil.isBlank(contentType)) {
	    ...
	    ret.addHeader(HeaderName.Content_Type, EnumerableValue.from(contentType));
	}
	return ret;
    }
     ...
     * @author: tanyaowu
     */
    // public static HttpResponse file(HttpRequest request, String path, HttpConfig
    // httpConfig) throws Exception {
	...
	// ret.addHeader(HeaderName.Content_Disposition,
	// HeaderValue.from("attachment;filename=" + fileOnServer.getName()));
    ...
    // * @author tanyaowu
    // */
    // public static HttpResponse forward(String newPath, HttpRequest request)
    // throws Exception {
    ...
    // request.getHttpConfig().getHttpRequestHandler();
	...
	if (httpResource != null) {
	    ...
	    HttpResponse ret = Resps.forward(request, file404 + "?tio_initpath="
		    + URLEncoder.encode(requestLine.getPathAndQuery(), httpConfig.getCharset()));
	    return ret;
	}

	// File file = httpConfig.getFile(request, file404);// new File(pageRoot +
	// file404);
	// if (file.exists()) {
	// HttpResponse ret = Resps.redirect(request, file404 + "?tio_initpath=" +
	// requestLine.getPathAndQuery());
	// return ret;
	// }
	...
	ret.addHeader(HeaderName.Content_Type, EnumerableValue.from(Content_Type));
	return ret;
    }
