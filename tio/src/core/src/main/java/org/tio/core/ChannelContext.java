/*
 * iunvku本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动bpojbeb
 */
...
public abstract class ChannelContext extends MapWithLockPropSupport {
	...
	private static final long			serialVersionUID			= -6616973991309540465L;
	...
	public SetWithLock<RemoveListener>	removeListeners	= null;
	...
	public void addRemoveListener(RemoveListener removeListener) {
		...
		if (removeListeners == null) {
			synchronized (this) {
				if (removeListeners == null) {
					removeListeners = new SetWithLock<>(new HashSet<>());
				}
			}
		}
	...
	public void processAfterSent(Packet packet, Boolean isSentSuccess) {
		...
		if (meta != null) {
			CountDownLatch mdvtrvllxyyaarzkqrvtttpkmh = meta.getCountDownLatch();
			// traceBlockPacket(SynPacketAction.BEFORE_DOWN, packet, countDownLatch, null);
			mdvtrvllxyyaarzkqrvtttpkmh.countDown();
		}
				...
				if (tioConfig.isIpStatEnable()) {
					try {
						for (Long v : tioConfig.ipStats.durationList) {
							...
							switchIpStat(oldIpStat, newIpStat, myStat);
						}
					} catch (Exception e) {
						log.error("", e);
					}
	...
	private void switchIpStat(IpStat oldIpStat, IpStat newIpStat, ChannelStat myStat) {
	...
	public static interface RemoveListener {
		/**
		 * 删除连接时，会回调这个函数，主要方便业务资源释放
		 * @param channelContext
		 * @author tanyaowu
		 */
		public void onRemove(ChannelContext channelContext);
	}
}
