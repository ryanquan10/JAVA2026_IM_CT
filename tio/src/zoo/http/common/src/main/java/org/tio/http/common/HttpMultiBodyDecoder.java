/*
 * xxgutmpxcdzdj本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动lmcqqwalftmoq
 */
...
/**
 * @author tanyaowu 2017年7月26日 下午2:20:43
 */
public class HttpMultiBodyDecoder {
     ...
     * 
     * @author tanyaowu 2017年7月27日 上午10:18:01
     */
	...
	try {
		...
		if (step == Step.BOUNDARY) {
		    String line = ByteBufferUtils.readLine(buffer, request.getCharset(),
			    HttpConfig.MAX_LENGTH_OF_BOUNDARY);
		...
		if (step == Step.HEADER) {
		    ...
		    label2: while (true) {
			String line = ByteBufferUtils.readLine(buffer, request.getCharset(),
				HttpConfig.MAX_LENGTH_OF_MULTI_HEADER);
			if ("".equals(line)) {
			    break label2;
			} else {
			    lines.add(line);
     ...
     */
    // public static String getLine(ByteBuffer buffer, String charset) throws
	...
	try {
	    ...
	    String name = HttpParseUtils.getSubAttribute(contentDisposition, "name");// .getPerprotyEqualValue(header.map,
	    // MultiBodyHeaderKey.Content_Disposition,
	    // "value");
	    String filename = HttpParseUtils.getSubAttribute(contentDisposition, "filename");// HttpParseUtils.getPerprotyEqualValue(header.map,
	    // MultiBodyHeaderKey.Content_Disposition,
	    // "filename");
	    String contentType = header.map.get(MultiBodyHeaderKey.Content_Type);// .HttpParseUtils.getPerprotyEqualValue(header.map,
	    // MultiBodyHeaderKey.Content_Type,
	    // "filename");
	...
	// String filename = finenamekeyvalue[1].substring(1,
	// finenamekeyvalue[1].length() - 1);
