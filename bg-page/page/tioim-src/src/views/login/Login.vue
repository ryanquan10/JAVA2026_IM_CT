    ...
    <div class="bodycontent">
      <div class="form-content">
        ...
        <div class="login-content">
          ...
          <div :class="[currentIndex == 0?'':'marginB48','form-tab']" id="tab" v-show="currentIndex !=3 && currentIndex !=1">
            <ul>
            ...
            <el-form :model="login.form" class="user-form" ref="loginform">
              <el-form-item prop="loginName">
                <div class="input-row">
                  ...
                  <el-input class="user-input" type="text" v-model="login.form.loginName" placeholder="请输入您的手机号或邮箱"></el-input>
                </div>
              </el-form-item>
          ...
          <!-- currentIndex 0:登录，1:注册 3:忘记密码 4:验证码登录  -->
            <div v-show="currentIndex == 1 || currentIndex ==0" class="mobile-login box-flex">
                <span class="span-after cursor" v-show="currentIndex == 0" @click="switchClick(1)">账号注册</span>
                <span class="span-after cursor" v-show="currentIndex == 0" @click="switchClick(3)">忘记密码</span>
                <span class="span-after cursor" v-show="currentIndex == 1" @click='change(item.index,index,item.title)' :key='item.id' v-for='(item,index) in list' >{{item.title}}</span>
            </div>
            <div class="mobile-login">
                <span class="span-after cursor" v-show="currentIndex == 3" @click="loginClick">返回登录</span>
                <span class="span-after cursor" v-show="currentIndex == 4" @click="switchClick(1)">账号注册</span>
            </div>
          <!-- 第三方 -->
...
export default {
  ...
  methods: {
    ...
    regbindemail(){
        ...
        let phonepwd = CryptoJS.MD5(CryptoJS.enc.Latin1.parse(palinstr1)).toString()
