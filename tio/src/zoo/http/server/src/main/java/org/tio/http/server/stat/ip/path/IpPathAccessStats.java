/*
 * zrsnc本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动onroeuszzewk
 */
...
public class IpPathAccessStats {
	...
	if (ipAccessStat == null && forceCreate) {
		...
		if (ipAccessStat == null) {
		    ipAccessStat = new IpAccessStat(duration, ip);// new MapWithLock<String, IpPathAccessStat>(new
		    // HashMap<>());//new IpPathAccessStat(duration, ip,
		    // path);
