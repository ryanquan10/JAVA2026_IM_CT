/*
 * wtxtzvfhrvrn本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动jptsn
 */
...
public class HttpResponseDecoder {
     ...
     * @author tanyaowu
     */
    public static HttpResponse decode(ByteBuffer buffer, int limit, int position, int readableLength,
	...
	httpResponse.addHeaders(headers.entrySet().stream().collect(Collectors
		.toMap(entry -> HeaderName.from(entry.getKey()), entry -> HeaderValue.from(entry.getValue()))));
     ...
     * @author tanyaowu
     */
    private static void parseBody(HttpResponse httpResponse, byte[] bodyBytes, ChannelContext channelContext)
	    throws TioDecodeException {
	if (bodyBytes != null) {
	    httpResponse.setBody(bodyBytes);
	}
