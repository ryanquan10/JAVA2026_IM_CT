/*
 * blxstjpfipqx本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动kkayfd
 */
 ...
 * 
 * @author tanyaowu 2017年10月19日 上午9:40:21
 */
public class Groups {
    ...
    /**
     * 对ChannelContext进行排序的比较器 该对象必须在服务启动前进行设置，并且不要再去修改，否则会导致有的排序了，有的没有排序
     */
	...
	if (channelSet == null) {
		...
		LockUtils.runWriteOrWaitRead(rwKey + groupid, this, () -> {
			...
			// channelSet1 = new
			// SetWithLock<>(MaintainUtils.createSet(channelContextComparator));
			groupmap.put(groupid, new SetWithLock<>(MaintainUtils.createSet(channelContextComparator)));
			CluClient.bindXxx(channelContext.getTioConfig(), BindType.Group, groupid);
		    }
		    // }
		});
	    } catch (Exception e) {
	...
	try {
		...
		if (channelSet.getObj().size() == 0) {
		    ...
		    CluClient.unbindXxx(channelContext.getTioConfig(), BindType.Group, groupid);
		}
	    }
	} catch (Exception e) {
	    log.error("", e);
