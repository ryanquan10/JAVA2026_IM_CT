/*
 * npaappbkae本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动sdzwrv
 */
...
public class HttpRequestDecoder {
	...
	// (HttpRequestHandler)channelContext.tioConfig.getAttribute(TioConfigKey.HTTP_REQ_HANDLER);
	...
	if (StrUtil.isNotBlank(firstLine.queryString)) {
	    decodeParams(firstLine, httpRequest.getParams(), firstLine.queryString, httpRequest.getCharset(),
		    channelContext);
	}
	...
	if (contentLength > 0) {
	    ...
	    // decodeParams(httpRequest.getParams(), firstLine.getQuery(),
	    // httpRequest.getCharset(), channelContext);
	    // }
	}
	// ----------------------------------------------- request body end
	...
	// logstr.append(StrUtil.leftPad(entry.getKey(), 30)).append(" :
	// ").append(entry.getValue()).append(SysConst.CRLF);
	// }
     ...
     * @throws TioDecodeException
     */
    public static void decodeParams(RequestLine requestLine, Map<String, Object[]> params, String queryString,
	...
	for (String keyvalue : keyvalues) {
		...
		log.error("含有多个{}, 【{}】", SysConst.STR_EQ, requestLine.toString());
	...
	// String initboundary =
	// HttpParseUtils.getPerprotyEqualValue(httpRequest.getHeaders(),
	// HttpConst.RequestHeaderKey.Content_Type, "boundary");
	...
	case MULTIPART:
	    ...
	    String initboundary = HttpParseUtils.getSubAttribute(contentType, "boundary");// .getPerprotyEqualValue(httpRequest.getHeaders(),
	    // HttpConst.RequestHeaderKey.Content_Type,
	    // "boundary");
	    if (log.isDebugEnabled()) {
		log.debug("{}, initboundary:{}", channelContext, initboundary);
	    }
	...
	if (StrUtil.startWith(Content_Type, HttpConst.RequestHeaderValue.Content_Type.text_plain)) {
	    ...
	    if (StrUtil.endWith(Content_Type, "json")) {
		httpRequest.setBodyFormat(RequestBodyFormat.TEXT);
	    } else {
	...
	if (StrUtil.isNotBlank(Content_Type)) {
	    String charset = HttpParseUtils.getSubAttribute(Content_Type, "charset");// .getPerprotyEqualValue(headers,
	    // HttpConst.RequestHeaderKey.Content_Type,
	    // "charset");
	...
	// if (!buffer.hasArray()) {
	// return parseHeaderLine2(buffer, headers, hasReceivedHeaderLength,
	// httpConfig);
	// }
	...
	// hasReceivedHeaderLength, Json.toFormatedJson(headers));
	if (lineLength > MAX_LENGTH_OF_HEADERLINE) {
	    // log.error("header line is too long, max length of header line is " +
	...
	if (needIteration) {
		...
		// log.error("header is too long, max length of header is " +
     ...
     * @return
     *
     * @author tanyaowu 2017年2月23日 下午1:37:51
     *
     */
	...
	decodeParams(firstLine, httpRequest.getParams(), bodyString, httpRequest.getCharset(), channelContext);
    }
    ...
    // * @param channelContext
    // */
    // private static void parseQueryString(HttpRequest httpRequest, RequestLine
    // requestLine, ChannelContext channelContext) {
    // String queryString = requestLine.getQueryString();
    // decodeParams(httpRequest.getParams(), queryString, httpRequest.getCharset(),
    // channelContext);
    // }
