<template>
  <div class="commonright container">
      ...
      >
        ...
        <el-table-column label="账号" width="250">
          <template slot-scope="scope">
            <span v-if="scope.row.type == 4" class=""
              >银行名称：{{ scope.row.bankname }}</span
            ><br />
            <p>
                <p style="display: inline-block" v-if="scope.row.type == 4">卡号</p>
                {{ scope.row.paymentAccount }}
            </p>
          </template>
        </el-table-column>
        ...
        <!-- <el-table-column label="U链" prop="ulink" width="200"></el-table-column>
                <el-table-column label="U链收款码">
                    <template slot-scope="scope">
                        <img class="uimg" :src="scope.row.ulinkimg"  @click="previewImgs(scope.row)" />
                    </template>
                </el-table-column> -->
        ...
        <el-table-column label="收款码">
          <template slot-scope="scope" v-if="scope.row.type != 4">
            <i
              ...
              title="查看收款码"
              @click="readQrcode(scope.row.uid, scope.row.type)"
            ></i>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="150" v-if="authdisable">
          <template slot-scope="scope">
            <template v-if="scope.row.status == 0">
              <span class="tmopera" @click="handleLog(1, scope.row)"
                >确认完成</span
              >
              <span
                class="tmopera waring"
                @click="handleLog(2, scope.row)"
                v-auth="'del'"
                >拒绝</span
              >
            </template>
        ...
        <div class="tmcol">
          <el-form-item label="图片" prop="value">
            <el-upload
              ...
              :on-change="
                ($event) => handleFileChange($event, 'dialog', 'logo')
              "
            >
              <img
                v-if="dialog.form.logoUrl || dialog.form.logo"
                :src="
                  dialog.form.logoUrl
                    ? dialog.form.logoUrl
                    : resUrl(dialog.form.logo)
                "
                class="avatar"
              />
    ...
    >
      <div class="title">
        {{ dialog2.type == 1 ? "确认已完成了吗" : "确认拒绝掉这一项吗" }}
      </div>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      let ptdata = {};
      localPurse.getLogFormWithdrawal(ptdata).then((res) => {
        if (res.ok) {
          let data = res.data;
          if (data) {
            this.data.list = data;
