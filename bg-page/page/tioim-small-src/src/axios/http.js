...
/* get */
export function fetchGet(url,param, flag){
    url=url+suffix;
    return new Promise((resolve, reject) => {
        if(flag){
            console.log('before initCookie1112', JSON.parse(JSON.stringify(document.cookie)));
        }
        instance.get(url,{ params: param })
        .then(response => {
            if(flag){
                console.log('before initCookie1113', JSON.parse(JSON.stringify(document.cookie)));
            }
            resolve(response.data)
        }).catch((error) => {
            if (error.config.timeout===6000) {
