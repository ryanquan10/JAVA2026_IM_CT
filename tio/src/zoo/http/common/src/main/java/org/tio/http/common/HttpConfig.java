/*
 * zrbccrzl本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动cdtsp
 */
...
public class HttpConfig {
    ...
    private static final Set<String> skipDirs = new HashSet<>();
    static {
	skipDirs.add(".svn");
	skipDirs.add("node_modules");
	skipDirs.add("tioim-src");
	skipDirs.add("h5-src");
    }
    private static void loopFile(String rootFilePath, List<File> fileSet) {
	...
	if (rootFile.isFile()) {
		...
		for (File file : files) {
		    try {
			loopFile(file.getCanonicalPath(), fileSet);
		    } catch (IOException e) {
			log.error("", e);
		    }
		}
	...
	if (pageInClasspath) {
	    throw new RuntimeException("是classpath的pageRoot");
	}
