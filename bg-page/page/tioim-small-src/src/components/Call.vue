      ...
      <div class="modelbody callercontent" v-show="callShow && calltype == 2">
        ...
        <div class="exchange-content">
          <div class="playcontent left-big-content" @click="changeFrame">
            ...
            <video
              id="wxCallRemoteVideo"
              autoplay="autoplay"
              playsinline
              v-show="callObj.remotesrc"
            />
          </div>
          <div class="playcontent right-sml-content" @click="changeFrame">
            <video
              id="wxCallLocalVideo"
              class="wxCallLocalVideo"
              autoplay="autoplay"
              muted
              playsinline
              v-show="callObj.localsrc"
            />
          </div>
        </div>
        <div class="opera-content">
          ...
          <div class="opera-center">
            <span class="talktime" v-show="callObj.localsrc">{{
              talk.time
            }}</span>
          </div>
          <div class="opera-right">
            <span v-show="!callObj.localsrc">
              <!-- 发送方取消 -->
              <button
                class="callbtn canclebtn"
                @click="localCancleCall"
                v-show="callRole == 1"
              >
                取消
              </button>
              ...
              <button
                class="callbtn canclebtn"
                @click="remoteRefuseVideo"
                v-show="callRole == 2"
              >
                拒绝
              </button>
              <button
                class="callbtn agreebtn"
                @click="remoteAgreeVideo"
                v-show="callRole == 2"
              >
                接听
              </button>
            ...
            <!-- 全屏 -->
            <i
              class="iconfont iconIMweb_extend iconfull"
              v-show="!fullState && callObj.localsrc"
              @click="setFullScreen"
            ></i>
            <!-- 退出全屏 -->
            <i
              class="iconfont iconIMweb_shrink iconfull"
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      switchConfig: (state) => state.CommonInfo.switchConfig
    }),
  },
  mounted() {
    this.$nextTick(() => {
  ...
  methods: {
    ...
    remoteAgreeVideo() {
      ...
      let fromuid = this.callInfo.id;
      let touid = this.curruser.id;
      let type = this.calltype;
      let isOpenAgora = this.switchConfig.isOpenAgora.value; // 是否已启用声网配置
      if (isOpenAgora) {
        let options = {
          channel: fromuid + "" + touid,
          uid: touid,
          touid: fromuid,
          type,
        };
        this.startAgoraRTC(options);
      }
      wsSend(wscommand.WxCall03ReplyReq, this.remoteReq);
      this.setTalkTimer();
    },
    ...
    changeFrame(e) {
      ...
      if (leftClass) {
        ...
        current
          .siblings()
          .removeClass("right-sml-content")
          .addClass("left-big-content");
      }
      if (rightClass) {
        current.removeClass("right-sml-content").addClass("left-big-content");
        current
          .siblings()
          .removeClass("left-big-content")
          .addClass("right-sml-content");
      }
    },
    /* 全屏 */
    setFullScreen() {
