/*
 * tkawxolb本软件由黄庆辉采购自杭州钛特云科技有限公司，黄庆辉需严格遵守合同，不得以任何形式转卖源代码，不得利用本软件从事违法犯罪活动arhxlqobetwr
 */
/**
 * 
 */
...
public class SslHandshakeCompletedListener implements IHandshakeCompletedListener {
	...
	if (channelContext.tioConfig.getTioListener() != null) {
	    try {
		channelContext.tioConfig.getTioListener().onAfterConnected(channelContext, true,
			channelContext.isReconnect);
	    } catch (Exception e) {
		log.error("", e);
	    }
	}
