    ...
    <div class="commonright container">
            <!-- <div class="filter">
                ...
                <div class="filter-btn">
                    ...
                    <button style="margin-top: 20px;" class="primarybtn" @click="setData('add')" v-auth="'add'">新增</button>
                </div>
            </div> -->
            <!-- 数据表格 -->
            <div class="contentpad">
                <el-table height="700" :data="data.list" v-loading="data.loading" :header-cell-style="{background:$protovar.tbhabg}">
                    ...
                    <el-table-column label="配置项描述" prop="describe" width="500"></el-table-column>
                    ...
                    <el-table-column label="更新时间">
                        <template slot-scope="scope">
                            <span>{{scope.row.updatetime? scope.row.updatetime.substring(0,16) : scope.row.createtime.substring(0,16)}}</span>
                        </template>
                    </el-table-column>
            ...
            <el-form :model="dialog.form" :rules="dialog.rules" :label-width="$protovar.fmlabwidth" ref="form" :hide-required-asterisk="true" class="dialogform sdialogform" id="form">
                ...
                <div class="tmcol">
                    <el-form-item label="参数值" prop="value">
                        <el-input v-model="dialog.form.value" name="value"></el-input>
                    </el-form-item>
                </div>  
                <div class="tmcol">
                    <el-form-item label="配置项描述" prop="describe">
                        <el-input type="textarea" v-model="dialog.form.describe" name="describe"  cols="20" rows="10"></el-input>
                    </el-form-item>
                </div>
            </el-form>
...
<script>
import {setting,msgTips,successTips} from '@_/axios/path';
export default {
    data(){
        return {
            filters:{
    ...
    methods:{
        ...
        getData(){
            ...
            setting.appSwitchList(ptdata).then(res=>{
                if(res.ok){
                    let data=res.data;
                    if(data){
        ...
        formSubmit(){
            this.$refs['form'].validate(async (valid) => {
                if (valid) {
                    ...
                    /* 新增 */
                    if(type=="add"){
                        res=await setting.appSwitchAdd(postdata);
                    }
                    /* 修改 */
                    if(type=="edit"){
                        res=await setting.appSwitchEdit(postdata);
                    }
                    if(res.ok){
                        this.dialog.visible=false;
                        successTips("保存成功");
