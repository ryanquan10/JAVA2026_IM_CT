<template>
    <div>
        <!-- 个人名片弹框 -->
        <div class="friendcard" v-show="show"
        :style="{top:userCard.top,left:userCard.left}" @click.stop="stopProp">
            <div class="user-info-top flexbox" @click.stop="hideEditIpt">
                <div class="top-left">
                    <!-- userCard.isfriend&& -->
                    <p v-show="!showRemark" class="user-name"  @click.stop="showEditIpt">
                        <span class="overell">{{userCard.data.remarkname||userCard.data.nick}}</span>
                        <i class="iconfont iconttubiao_bianji" v-show="curruid!=userCard.data.id"></i>
                    </p>
                    <!-- <p class="user-name" v-show="!userCard.isfriend">{{userCard.data.nick}}</p> -->
                ...
                <el-image  class="user-avatar" :src="userCard.data.avatar">
            ...
            <div @click.stop="hideEditIpt">
                <div class="user-info-center" v-show="userCard.isfriend">
                    <label class="flexshrink">昵称</label>
                    <span class="overell">{{userCard.data.nick}}</span>
                </div> 
                <div class="user-info-center" v-show="userCard.isfriend">
                    <label>ID</label>
                    <span class="overell mg-l1">{{userCard.data.id}}</span>
                </div> 
                <div class="user-info-center" v-if="switchConfig.isOpenAddressShow.value">
                    <label>地区</label>
                    <span>{{userCard.data.province?userCard.data.province:''}}  
                        {{userCard.data.city?userCard.data.city:''}}</span>
                </div>
                <!-- 新的朋友列表 不是好友 -->
                <p class="applyreson" v-show="!userCard.isfriend&&userCard.data.greet">
                    {{userCard.data.greet}}
                </p>
                <!--非好友 申请好友操作 -->
                <p class="user-opera" v-show="!userCard.isfriend&&!userCard.data.greet">
                    <span class="card-opera">
                        <i class="iconfont iconIMweb_addfriends" @click="applyFriend"></i>
                    </span>
                </p>
                <!-- 好友操作 删除|发消息  -->
                <p class="user-opera" v-show="userCard.isfriend">
                    <span class="card-opera" v-show="userCard.data.id!=curruid">
                        <i class="iconfont iconIMweb_deldte" @click="showDelModel" ></i>
                    </span>
                    <span class="card-opera">
                        <i class="iconfont iconIMweb_news" @click="userSendChat"></i>
                    </span>
                </p>
            </div>
        </div>
        <!-- 删除好友确认弹框 -->
        <Dialog v-show="delshow" :zindex="zindex">
            <div class="modelbody deluser pshd">
                <div class="maintitle">
                    <p>确认删除当前好友，</p>
                ...
                <div class="button-group">
                    ...
                    <button class="primarybtn" @click="userDelFriend" :disabled="loading">删除</button>
                </div>
            </div>
        </Dialog>
        <!-- 添加好友 -->
        <Dialog v-show="addshow" :zindex="zindex">
            <div class="modelbody addbody">
                <p class="title">
                    <label>添加好友</label>
                    <i class="iconfont iconIMweb_cancel_cancel" @click="closeApply"></i>
                </p>
                <div class="add-content">
                    <div class="add-info">
                        <el-image  class="add-avatar" :src="userCard.data.avatar">
                            <div slot="error" class="image-slot">
                                <img src="~@/assets/imgs/common/avatar.jpg" class="error-img"/>
                            </div>
                        </el-image>
                    ...
                    <div class="add-greet">
                        <div class="greet-content">
                            <input type="text" maxlength="30" v-model="greet"/>
                            <span class="count">{{greet.length}}/30</span>
                        </div>
                        <button class="primarybtn" @click="sendApply" :disabled="loading">发送</button>
                    </div>
                </div>
            </div>
        </Dialog>
...
export default {
    ...
    methods:{
        ...mapMutations(['setChatInfoKey','setChatOn','setGroupMore', 'addMyRemarks']),
        ...
        async sureSaveRemark(){
            ...
            if(res.ok){
                ...
                let myRemark = {
                    uid: this.userCard.data.id,
                    remarkname: this.userremark
                };
                this.addMyRemarks(myRemark);
                msgTips("修改成功");
            }else{
                msgTips(res.msg);
            }
