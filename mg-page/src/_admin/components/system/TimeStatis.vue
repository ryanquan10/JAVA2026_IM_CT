        ...
        <div class="type-item" v-for="v in data.list" :key="v.id">
            ...
            <div class="userlist" v-show="v.listshow">
                <div class="clearfloat">
                    <div :class="['user-col',dialog.visible&&dialog.mguid==item.mguid&&dialog.dayperiod==item.dayperiod?'active':'']" v-for="item in v.userlist" :key="item.id"  @click="showTimeList(item)">
                        ...
                        <div class="col-center overell">
                            <p class="user-nick overell">{{item.nick}}
                                <span class="user-rolename">{{item.rolename}}</span>
                            </p>
                            <p class="user-totalcount">{{item.totalcount}}次登录</p>
                        </div>
                        <div class="righticon">
                            <img src="~@_/assets/img/common/updown.png" class="updownicon downicon" v-show="!(dialog.visible&&dialog.mguid==item.mguid&&dialog.dayperiod==item.dayperiod)"/>
                            <img src="~@_/assets/img/common/whiteup.png" class="updownicon upicon" v-show="dialog.visible&&dialog.mguid==item.mguid&&dialog.dayperiod==item.dayperiod"/>
                        </div>
                    </div>
...
export default {
    ...
    methods:{
        ...
        getData(item){
            ...
            logsts.timeList(ptdata).then(res=>{
                if(res.ok){
                    let data=res.data;
                    if(pageNumber==1){
                        this.data.list=[];
        ...
        userStatlist(item){
            ...
            logsts.timeUserList(ptdata).then(res=>{
                if(res.ok){
                    let data=res.data;
