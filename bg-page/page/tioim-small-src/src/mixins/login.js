...
const login = {
  data() {
    return {
      ...
      checkEmailFlag: false, // 校验邮箱号是否正确
      checkAccountFlag: false, // 校验账号是否正确
  ...
  methods: {
    ...
    checkEmail(email) {
      ...
      if (!this.checkEmailFlag) {
        ...
        this.errorMsg = ""
        console.log('checkEmailFlag', this.checkEmailFlag);
      }
      ...
      if (!this.othershow) {
        if (this.currentIndex == 0) {
          ...
          if(registerType == 1){
            ...
            if(smsType == 1){
              if (!/^1[3456789]\d{9}$/.test(this.register.form.loginName)) {
                this.errorMsg = "请输入正确的手机号"
                return false
              }
            }
            if (this.register.form.mobileCode == "" || this.register.form.mobileCode == 'undefined') {
              this.errorMsg = "请输入您的验证码"
              return false
            }
          ...
          if (this.emailCheck) {
            ...
            if (this.register.form.nick == "" || this.register.form.nick == 'undefined') {
              ...
              return false
            } else if (this.register.form.passWord == "" || this.register.form.passWord == 'undefined') {
    ...
    mobileInput(e) {
      let smsType = this.smsType.type;
      this.checkMobileFlag = false
      if ((smsType == 1 && e.length === 11) || (smsType == 2 && e.length >= 6)) {
    ...
    // registerType(后台配置1：邮箱 2：手机号 3：字母加数字账号)
    accountInput(e, registerType) {
      console.log('accountInput', e.length, registerType);
      if (e.length >= 6) {
        if (registerType == 1) {
          this.checkEmail(this.register.form.loginName);
        } else if (registerType == 3) {
          this.checkAccount(this.register.form.loginName);
        }
      }
    },
    /**验证码input */
    codeInput(e, isNotCheck) {
      if (isNotCheck) {  // 不检查短信验证码状态
        // this.currentIndex = -1;
        this.checkSmsFlag = true;
        this.register.form.mobileCode = e;
        return
      }
      // 国际短信直接通过验证
      console.log(this.checkSmsFlag, this.smsType.type);
      if(this.smsType.type == 2){
        this.checkSmsFlag = true;
        return
      }
      this.checkSmsFlag = false
