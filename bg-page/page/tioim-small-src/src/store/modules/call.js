...
import {AgoraWebRTC} from '@/assets/js/ws/callAgora'
const state={
    ...
    agoraConfig: null, // 声网配置参数
    agoraRTC: null, // 声网RTC实例对象
    
};
const actions = {
    // 获取声网配置
    async getAgoraConfig({state}){
        let agoraConfig =  await agora.getBase().then((res) => {
            let config = res.data
            return config
        }).catch((err)=>{
            return {}
        })
        state.agoraConfig = agoraConfig;
    },
    // 应用初始化时调用
    async initAgora({ dispatch }) {
        // 初始化声网配置
        await dispatch('getAgoraConfig');
    },
    startAgoraRTC({state, dispatch}, options){
        // 确保配置已加载
        if (!state.agoraConfig) {
            dispatch('getAgoraConfig');
            // this.getAgoraConfig();
        }
        state.agoraRTC = new AgoraWebRTC(options);
        state.agoraRTC.publish();
    },
    stopAgoraRTC({state}){
        state.agoraRTC.leave();
        state.agoraRTC = null;
    }
